...
#define DEFINE_AUTOMATION_TYPE_FUNCTION(ctype, typewrapper, oleautomationtype) \
	...
	{ \
		...
		static void* GetT(const ctype& t) noexcept \
		{ \
			return (void*)&t; \
		} \
	};
	...
	struct _ATL_AutomationType<BSTR>
	{
		...
		enum { type = VT_BSTR };
		static void* GetT(_In_ const BSTR& t) noexcept
		{
			return t;
		}
	};
	...
	struct _ATL_AutomationType<LPUNKNOWN>
	{
		...
		static void* GetT(_In_ const LPUNKNOWN& t) noexcept
		{
			return t;
		}
	};
	...
	struct _ATL_AutomationType<LPDISPATCH>
	{
		...
		static void* GetT(_In_ const LPDISPATCH& t) noexcept
		{
			return t;
		}
	};
...
public:
	CComSafeArrayBound(
		...
		_In_ LONG lLowerBound = 0) noexcept
	{
		cElements = ulCount;
		lLbound = lLowerBound;
	}
	CComSafeArrayBound& operator=(_In_ const CComSafeArrayBound& bound) noexcept
	{
		cElements = bound.cElements;
		lLbound = bound.lLbound;
		return *this;
	}
	CComSafeArrayBound& operator=(_In_ ULONG ulCount) noexcept
	{
		cElements = ulCount;
		lLbound = 0;
		return *this;
	...
	ULONG SetCount(_In_ ULONG ulCount) noexcept
	{
		cElements = ulCount;
		return cElements;
	}
	LONG GetLowerBound() const noexcept
	{
		return lLbound;
	}
	LONG SetLowerBound(_In_ LONG lLowerBound) noexcept
	{
		lLbound = lLowerBound;
		return lLbound;
	}
	LONG GetUpperBound() const noexcept
	{
...
public:
// Constructors
	CComSafeArray() noexcept : m_psa(NULL)
	{
	}
...
// overloaded operators
	...
	operator const SAFEARRAY *() const noexcept
	{
		return m_psa;
	}
	operator LPSAFEARRAY() noexcept
	{
		return m_psa;
	}
	LPSAFEARRAY* GetSafeArrayPtr() noexcept
	{
		return &m_psa;
	}
...
HRESULT CComSafeArray<LPUNKNOWN>::SetAt(
	...
	_In_ BOOL bAddRef)
{
	ATLENSURE_RETURN_HR((pUnk != NULL), E_UNEXPECTED);
	ATLASSUME(m_psa != NULL);
	LONG lLBound = GetLowerBound();
	ATLASSERT(lIndex >= lLBound);
	ATLASSERT(lIndex <= GetUpperBound());
...
HRESULT CComSafeArray<LPDISPATCH>::SetAt(
	...
	_In_ BOOL bAddRef)
{
	ATLENSURE_RETURN_HR((pDisp != NULL), E_UNEXPECTED);
	ATLASSUME(m_psa != NULL);
	LONG lLBound = GetLowerBound();
	ATLASSERT(lIndex >= lLBound);
	ATLASSERT(lIndex <= GetUpperBound());
