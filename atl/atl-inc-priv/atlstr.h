...
{
public:
	CAtlStringMgr(_In_opt_ IAtlMemMgr* pMemMgr = NULL) noexcept :
		m_pMemMgr( pMemMgr )
	{
	...
	void SetMemoryManager(_In_ IAtlMemMgr* pMemMgr) noexcept
	{
...
public:
	...
	virtual void Free(_In_ CStringData* pData) noexcept
	{
...
{
public:
	static int tclen(_In_z_ LPCSTR p) noexcept
	{
		ATLASSERT(p != NULL);
	...
	static _CharType* _strrev(_Inout_opt_z_ _CharType* psz) noexcept
	{
	...
	_Ret_maybenull_z_ static LPCSTR strpbrk(
		_In_z_ LPCSTR p,
		_In_z_ LPCSTR lpszCharSet) noexcept
	{
		int nRet=0;
	...
	static _CharType* CharNext(_In_ const _CharType* p) noexcept
	{
		return AtlCharNext(p);
	}

	static int IsDigit(_In_ _CharType ch) noexcept
	{
		WORD type;
	...
	static int IsSpace(_In_ _CharType ch) noexcept
	{
		WORD type;
		GetStringTypeExA(GetThreadLocale(), CT_CTYPE1, &ch, 1, &type);
	...
	static int StringCompare(
		...
		_In_z_ const _CharType* pstrOther) noexcept
	{
	...
	_Ret_maybenull_z_ static LPCSTR StringFindString(
		...
		_In_z_ LPCSTR pstrMatch) noexcept
	{
	...
	_Ret_maybenull_z_ static LPSTR StringFindString(
		...
		_In_z_ LPCSTR pszMatch) noexcept
	{
	...
	static int StringSpanIncluding(
		...
		_In_z_ const _CharType* pstrSet) noexcept
	{
	...
	static _CharType* StringUppercase(_Inout_z_ _CharType* psz) noexcept
	{
		return CharUpperA( psz );
	}

	_ATL_INSECURE_DEPRECATE("ChTraitsOS::StringLowercase must be passed a buffer size")
	static _CharType* StringLowercase(_Inout_z_ _CharType* psz) noexcept
	{
	...
	static _CharType* StringReverse(_Inout_z_ _CharType* psz) noexcept
	{
		return _strrev( psz );
	}
	...
	static int GetBaseTypeLength(_In_z_ const char* pszSrc) noexcept
	{
	...
	static int GetBaseTypeLength(_In_z_ const wchar_t* pszSrc) noexcept
	{
		// Returns required buffer length in XCHARs
	...
	static void ConvertToBaseType(
		...
		_In_ int nSrcLength = -1) noexcept
	{
	...
	static void ConvertToOem(_Inout_z_ _CharType* pstrString) noexcept
	{
	...
	static void ConvertToAnsi(_Inout_z_ _CharType* pstrString) noexcept
	{
	...
	static void FloodCharacters(
		_In_ _CharType ch,
		_In_ int nLength,
		_Out_writes_bytes_all_(nLength) _CharType* pstr) noexcept
	{
	...
	static DWORD FormatMessage(
		...
		_In_ DWORD nSize,
		_In_opt_ va_list* pArguments) noexcept
	{
		return ::FormatMessageA(dwFlags, lpSource, dwMessageID, dwLanguageID,
				pstrBuffer, nSize, pArguments);
	}

	static int SafeStringLen(_In_opt_z_ const char* psz) noexcept
	{
		// returns length in bytes
		return (psz != NULL) ? static_cast<int>(strlen(psz)) : 0;
	}

	static int SafeStringLen(_In_opt_z_ const wchar_t* psz) noexcept
	{
		// returns length in wchar_ts
		return (psz != NULL) ? static_cast<int>(wcslen(psz)) : 0;
	}

	static int GetCharLen(_In_opt_z_ const wchar_t*) noexcept
	{
		// returns char length
		return 1;
	}
	static int GetCharLen(_In_z_ const char* psz) noexcept
	{
...
public:
	static int tclen(_In_opt_z_ const wchar_t*) noexcept
	{
		return 1;
	}
	_Ret_maybenull_z_ static LPCWSTR strchr(
		_In_z_ LPCWSTR p,
		_In_ wchar_t ch) noexcept
	{
		return AtlstrchrT(p,ch);
	}
	...
	_Ret_maybenull_z_ static wchar_t* _strrev(_Inout_opt_z_ wchar_t* psz) noexcept
	{
		// Optimize NULL, zero-length, and single-char case.
	...
	_Ret_maybenull_z_ static LPCWSTR strstr(
		_In_z_ LPCWSTR pStr,
		_In_z_ LPCWSTR pCharSet) noexcept
	{
		return strstrT< ChTraitsOS<XCHAR> >(pStr,pCharSet);
	}
	static int strspn(
		_In_z_ const wchar_t* psz,
		_In_z_ const wchar_t* pszCharSet) noexcept
	{
		int nRet = 0;
		const wchar_t* p = psz;
		while (*p != 0)
	...
	_Ret_maybenull_z_ static LPCWSTR strpbrk(
		_In_z_ LPCWSTR psz,
		_In_z_ LPCWSTR pszCharSet) noexcept
	{
		const wchar_t* p = psz;
		while (*p != 0)
		{
	...
	static wchar_t* CharNext(_In_ const wchar_t* p) noexcept
	{
		return AtlCharNext(p);
	}

	static int IsDigit(_In_ wchar_t ch) noexcept
	{
		WORD type;
	...
	static int IsSpace(_In_ wchar_t ch) noexcept
	{
		WORD type;
		GetStringTypeExW(0, CT_CTYPE1, &ch, 1, &type);
		return (type & C1_SPACE) == C1_SPACE;
	...
	static int StringCompare(
		...
		_In_z_ const wchar_t* pstrOther) noexcept
	{
	...
	_Ret_maybenull_z_ static LPCWSTR StringFindString(
		...
		_In_z_ LPCWSTR pstrMatch) noexcept
	{
		return strstr(pstrBlock, pstrMatch);
	}
	...
	_Ret_maybenull_z_ static LPWSTR StringFindString(
		...
		_In_z_ LPCWSTR pszMatch) noexcept
	{
	...
	_Ret_maybenull_z_ static LPCWSTR StringFindChar(
		_In_z_ LPCWSTR pstrBlock,
		_In_ wchar_t pstrMatch) noexcept
	{
	...
	static int StringSpanIncluding(
		...
		_In_z_ const wchar_t* pszSet) noexcept
	{
	...
	_ATL_INSECURE_DEPRECATE("ChTraitsOS::StringUppercase must be passed a buffer size")
	static wchar_t* StringUppercase(_Inout_z_ wchar_t* psz) noexcept
	{
		CharUpperW(psz);
		return psz;
	}
	...
	_ATL_INSECURE_DEPRECATE("ChTraitsOS::StringUppercase must be passed a buffer size")
	static wchar_t* StringLowercase(_Inout_z_ wchar_t* psz) noexcept
	{
		CharLowerW(psz);
		return psz;
	}
	...
	_Ret_maybenull_z_ static wchar_t* StringReverse(_Inout_z_ wchar_t* psz) noexcept
	{
		return _strrev(psz);
	}
	...
	*/
	...
	static void FloodCharacters(
		_In_ wchar_t ch,
		_In_ int nLength,
		_Out_writes_all_(nLength) wchar_t* pstr) noexcept
	{
		// nLength is in XCHARs
...
public:
	...
	static IAtlStringMgr* GetDefaultManager() noexcept
	{
        ...
        if (pmemmgr != NULL && pmemmgr->FGetPtrFromThread((void**) &pstrmgr) && pstrmgr != NULL)
            return pstrmgr;
#endif
