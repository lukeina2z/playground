...
inline HRESULT AtlGenXMLValue(__in IWriteStream *pStream, __in T *pVal)
{
    ...
    CHttpResponseOutNotSafe::PPNotSafeWriteT(&s, pVal);
    return S_OK;
...
//      hr = (s.Write(wszWrite) == TRUE ? S_OK : E_FAIL);
        CHttpResponseOutNotSafe::PPNotSafeWrite(&s, wszWrite);
        hr = S_OK;
...
public:
	...
	CSkipHandler(ISAXContentHandler *pParent = NULL, ISAXXMLReader *pReader = NULL)
        : m_pParent(pParent), m_spReader(pReader), m_dwReset(1)
    {
    }

	void Reset(DWORD dwCnt = 1)
	{
	    //  fix: second time the object is used m_dwReset is on 0 (see endElement)
	    //  calling class should reset the counter to 1
	    m_dwReset = dwCnt;
	}
		 ...
		 int /*cchQName*/)
	{
		if (EnableReset() == 0)
		{
            m_spReader->putContentHandler(m_pParent);
		}
...
public:
	...
	HRESULT GenerateFault(IWriteStream *pWriteStream)
	{
		...
		_ATLTRY
		{
			...
			CStringA strFault;
			strFault.Format(s_szErrorFormat, wszFaultCode, m_strFaultString.GetString(),
				m_strFaultActor.GetLength() ? "<faultactor>" : "", m_strFaultActor.GetString(),
...
private:
	...
	HRESULT GenerateArrayInfo(const _soapmapentry *pEntry, const int *pDims, IWriteStream *pStream)
	{
		...
		for (int i=1; i<=pDims[0]; i++)
		{
            CHttpResponseOutNotSafe::PPNotSafeWrite(&s, pDims[i]);
            //if (!s.Write(pDims[i]) ||
            if ((!TRUE) ||
    ...
    virtual
    bool IsHeaderNullable(const _soapmapentry& refEntry)
    {
        return (refEntry.dwFlags & (SOAPFLAG_NULLABLE | SOAPFLAG_NOMARSHAL))==0;
    }
			...
			for (size_t i=0; pEntries[i].nHash != 0; i++)
			{
					...
					((pEntries[i].dwFlags & dwExcludeFlags)==0))
				{
					if (state.vec.GetBit(i) == false)
					{
                        ...
                        //  fixed check on required soap header. Structured headers don't have SOAPFLAG_NULLABLE set even when
                        //  the header is optional. It is ok to omit them but not if the header is required
                        //
                        if (!IsHeaderNullable(pEntries[i]) || (pEntries[i].dwFlags & SOAPFLAG_MUSTUNDERSTAND))
						{
...
public:
	...
	virtual HRESULT BeginParse(IStream *pStream)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			// ATL has a bug where if an empty body is used, the wrong map is used to call the function
			// as a result all the input members point to garbage memory. We detect this and block before
			// the function is called.
			//
			else if(m_stateStack.GetCount() != 0 && m_stateStack[0].pMap && m_stateStack[0].pMap->mapType != SOAPMAP_FUNC)
			{
				hr = E_FAIL;
			}
		}
    ...
    //  return the soapmap for method being called
    //
    const _soapmap* GetSoapMethodMap()
    {
        const _soapmap* pMap = NULL;

        if (!m_stateStack.IsEmpty())
        {
            pMap = m_stateStack[0].pMap;
        }

        return pMap;
    }

    virtual HRESULT CallFunctionInternal()
	{
		HRESULT hr = E_FAIL;
...
public:
	...
	HTTP_CODE InitializeHandler(AtlServerRequest *pRequestInfo, IServiceProvider *pProvider)
	{
		...
		if (m_hcErr == HTTP_SUCCESS)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				m_hcErr = pT->ValidateAndExchange();
    ...
    virtual void ProcessResponseContent(LPCSTR)
    {
        // Nothing to do
    }
	...
	HTTP_CODE HandleRequest(AtlServerRequest *pRequestInfo, IServiceProvider * /*pProvider*/)
	{
        ...
        //
        // Else part is removed so that ppsoaphandler can return error XML along with the
        // 500 error.
        //
		...
		{
			Cleanup();
			ProcessResponseContent((LPCSTR)HttpResponse.m_strContent);
...
private:
	...
	HRESULT ConnectToServer()
	{
		...
		m_hInternet = InternetOpen(
            m_strUserAgentString.IsEmpty() ? ATLSOAPINET_CLIENT : m_strUserAgentString,
...
public:
    ...
    void SetUserAgentString(LPCTSTR str) { m_strUserAgentString = str; }
...
private:
	...
	HRESULT ConnectToServer()
	{
        ...
        // Condition added to not intefere with legacy cases which used 3.0 version of IServerXMLHTTP
		hr = m_spHttpRequest.CoCreateInstance(__uuidof(ServerXMLHTTP60));
		if (hr != S_OK)
		{
			return hr;
		}
        ...
        if(VT_EMPTY != m_varClientCertName.vt)
        {
            // Passport (2003-01-28): Added to have SSL with client-auth challenge succeed
            hr = m_spHttpRequest->setOption(SXH_OPTION_SELECT_CLIENT_SSL_CERT,
                                            m_varClientCertName);
            if (hr != S_OK)
                return hr;
        }
...
public:
    ...
    CComVariant m_varClientCertName;
	CSoapFault m_fault;

	CSoapMSXMLInetClient(LPCTSTR szUrl)
        :m_dwTimeout(0), m_errorState(SOAPCLIENT_SUCCESS), m_varClientCertName(L"")
...
private:
	...
	HRESULT GenerateWSDLHelper(const _soapmap *pMap, WSDLMAP& structMap, WSDLMAP& enumMap)
	{
		...
		{
            if (pEntries[i].nVal == SOAPTYPE_UNK && pMap->mapType != SOAPMAP_ENUM)
			{
	...
	HTTP_CODE GetSoapDims(const _soapmapentry *pEntry)
	{
		...
		if (pEntry->pDims[0] != 0)
		{
			if (SUCCEEDED(m_pWriteStream->WriteStream("[", 1, NULL)))
			{
				...
				{
                    CHttpResponseOutNotSafe::PPNotSafeWrite(&m_writeHelper, pEntry->pDims[i]);
...
public:
    ...
    virtual CStringA OnGetURLInner(bool bHttps, const char* szServer, const char* szPort, const char* szPath)
    {
        CStringA strUrl;
        strUrl.Format("http%s://%s:%s%s?Handler=%s", bHttps ? "s" : "", szServer, szPort, szPath, GetHandlerName());
        return strUrl;
    }
		...
		if (m_spHttpServerContext->GetServerVariable("URL", szURL, &dwUrlSize) != FALSE)
		{
			if (m_spHttpServerContext->GetServerVariable("SERVER_NAME", szServer, &dwServerSize) != FALSE)
			{
				...
				if (m_spHttpServerContext->GetServerVariable("SERVER_PORT", szPort, &dwPortLen) != FALSE)
				{
					_ATLTRY
					{
                        CStringA strUrl = OnGetURLInner(bHttps, szServer, szPort, szURL);
	...
	{
        CHttpResponseOutNotSafe::PPNotSafeWrite(&m_writeHelper, m_pFuncs[m_nFunc]->pEntries[m_nParam].pDims[1]);
	...
	{
		const _soapmap *pMap = m_structMap.GetValueAt(m_currUDTPos);
        CHttpResponseOutNotSafe::PPNotSafeWrite(&m_writeHelper, pMap->pEntries[m_nCurrUDTField].pDims[1]);
	...
	{
		const _soapmapentry *pEntry = m_headerMap.GetValueAt(m_currHeaderPos);
        CHttpResponseOutNotSafe::PPNotSafeWrite(&m_writeHelper, pEntry->pDims[1]);
...
template <class THandler, const char *szHandlerName, class TGenerator>
class CSDLGenerator :
    public TGenerator,
    public IRequestHandlerImpl< CSDLGenerator<THandler, szHandlerName, TGenerator> >,
    public CComObjectRootEx<CComSingleThreadModel>
{
private:

public:
    typedef CSDLGenerator<THandler, szHandlerName, TGenerator> _sdlGenerator;
