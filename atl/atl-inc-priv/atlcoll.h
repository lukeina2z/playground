...
#include <atldef.h>
#include <atlbase.h>
#ifdef MALLOC_PERFTUNING
#define malloc  PPMALLOC
#define free    PPFREE
#define realloc  PPREALLOC
...
inline void* __cdecl PPMALLOC(size_t n)
{
    IPPPerThreadMemMgr *pmemmgr = ::GetPerThreadHeapManagerEx();

    if (pmemmgr)
        return pmemmgr->Allocate(n);

    return  ::HeapAlloc(::GetProcessHeap(), 0, n);

}

inline void __cdecl PPFREE(void* p)
{
    IPPPerThreadMemMgr *pmemmgr = ::GetPerThreadHeapManagerEx();

    if (pmemmgr)
        return pmemmgr->Free(p);

   ::HeapFree(::GetProcessHeap(), 0, p);
}

inline void* __cdecl PPREALLOC(void* p, size_t n)
{
    IPPPerThreadMemMgr *pmemmgr = ::GetPerThreadHeapManagerEx();

    if (pmemmgr)
        return pmemmgr->Reallocate(p, n);
...
{
public:
	static ULONG Hash(_In_ const T& element) noexcept
	{
...
{
public:
	static ULONG Hash(_In_ INARGTYPE bstr) noexcept
	{
		ULONG nHash = 0;
...
private:
	...
	static void CallDestructors(
		...
		_In_ size_t nElements) noexcept;
...
private:
	...
	CAtlArray(_In_ const CAtlArray&) noexcept;
	CAtlArray& operator=(_In_ const CAtlArray&) noexcept;
};
...
{
public:
	CInterfaceArray() noexcept
	{
	}

private:
	// Private to prevent use
	CInterfaceArray(_In_ const CInterfaceArray&) noexcept;
	CInterfaceArray& operator=(_In_ const CInterfaceArray&) noexcept;
};
...
{
public:
	CAutoPtrArray() noexcept
	{
	}

private:
	// Private to prevent use
	CAutoPtrArray(_In_ const CAutoPtrArray&) noexcept;
	CAutoPtrArray& operator=(_In_ const CAutoPtrArray&) noexcept;
};
...
{
public:
	CHeapPtrArray() noexcept
	{
	}

private:
	// Private to prevent use
	CHeapPtrArray(_In_ const CHeapPtrArray&) noexcept;
	CHeapPtrArray& operator=(_In_ const CHeapPtrArray&) noexcept;
};

template< typename E, class ETraits >
inline size_t CAtlArray< E, ETraits >::GetCount() const noexcept
{
	return( m_nSize );
}

template< typename E, class ETraits >
inline bool CAtlArray< E, ETraits >::IsEmpty() const noexcept
{
	return( m_nSize == 0 );
}

template< typename E, class ETraits >
inline void CAtlArray< E, ETraits >::RemoveAll() noexcept
{
	SetCount( 0, -1 );
}
...
template< typename E, class ETraits >
inline const E* CAtlArray< E, ETraits >::GetData() const noexcept
{
	return( m_pData );
}

template< typename E, class ETraits >
inline E* CAtlArray< E, ETraits >::GetData() noexcept
{
	return( m_pData );
}
...
template< typename E, class ETraits >
CAtlArray< E, ETraits >::CAtlArray()  noexcept:
	m_pData( NULL ),
	m_nSize( 0 ),
	m_nMaxSize( 0 ),
	m_nGrowBy( 0 )
...
template< typename E, class ETraits >
void CAtlArray< E, ETraits >::FreeExtra() noexcept
{
	ATLASSERT_VALID(this);
...
private:
	...
	private:
		CNode(_In_ const CNode&) noexcept;
	};

public:
	CAtlList(_In_ UINT nBlockSize = 10) noexcept;
	...
	E RemoveHead();
	E RemoveTail();
	void RemoveHeadNoReturn() noexcept;
	void RemoveTailNoReturn() noexcept;
	...
	void SetAt(
		_In_ POSITION pos,
		/* _In_ */ INARGTYPE element);
	void RemoveAt(_In_ POSITION pos) noexcept;
	...
	POSITION Find(
		/* _In_ */ INARGTYPE element,
		_In_opt_ POSITION posStartAfter = NULL) const noexcept;
	POSITION FindIndex(_In_ size_t iElement) const noexcept;

	void MoveToHead(_In_ POSITION pos);
	void MoveToTail(_In_ POSITION pos);
	void SwapElements(
		_In_ POSITION pos1,
		_In_ POSITION pos2) noexcept;
...
private:
	...
	CNode* NewNode(
		...
		_In_opt_ CNode* pPrev,
		_In_opt_ CNode* pNext);
	void FreeNode(_Inout_ CNode* pNode) noexcept;
...
private:
	// Private to prevent use
	CAtlList(_In_ const CAtlList&) noexcept;
	CAtlList& operator=(_In_ const CAtlList&) noexcept;
};
...
{
public:
	CInterfaceList(_In_ UINT nBlockSize = 10) noexcept :
...
private:
	// Private to prevent use
	CInterfaceList(_In_ const CInterfaceList&) noexcept;
	CInterfaceList& operator=(_In_ const CInterfaceList&) noexcept;
};
...
{
public:
	CAutoPtrList(_In_ UINT nBlockSize = 10) noexcept :
...
private:
	// Private to prevent use
	CAutoPtrList(_In_ const CAutoPtrList&) noexcept;
	CAutoPtrList& operator=(_In_ const CAutoPtrList&) noexcept;
};
...
{
public:
	CHeapPtrList(_In_ UINT nBlockSize = 10) noexcept :
...
private:
	// Private to prevent use
	CHeapPtrList(_In_ const CHeapPtrList&) noexcept;
	CHeapPtrList& operator=(_In_ const CHeapPtrList&) noexcept;
};

template< typename E, class ETraits >
inline size_t CAtlList< E, ETraits >::GetCount() const noexcept
{
	return( m_nElements );
}

template< typename E, class ETraits >
inline bool CAtlList< E, ETraits >::IsEmpty() const noexcept
{
...
template< typename E, class ETraits >
inline POSITION CAtlList< E, ETraits >::GetHeadPosition() const noexcept
{
	return( POSITION( m_pHead ) );
}

template< typename E, class ETraits >
inline POSITION CAtlList< E, ETraits >::GetTailPosition() const noexcept
{
...
template< typename E, class ETraits >
inline const E& CAtlList< E, ETraits >::GetPrev(_Inout_ POSITION& pos) const noexcept
{
	CNode* pNode;
...
template< typename E, class ETraits >
CAtlList< E, ETraits >::CAtlList(_In_ UINT nBlockSize) noexcept :
	m_nElements( 0 ),
	m_pHead( NULL ),
	m_pTail( NULL ),
...
template< typename E, class ETraits >
void CAtlList< E, ETraits >::RemoveAll() noexcept
{
	while( m_nElements > 0 )
	{
		CNode* pKill = m_pHead;
...
template< typename E, class ETraits >
CAtlList< E, ETraits >::~CAtlList() noexcept
{
	RemoveAll();
	ATLASSUME( m_nElements == 0 );
}
...
template< typename E, class ETraits >
void CAtlList< E, ETraits >::FreeNode(_Inout_ CNode* pNode) noexcept
{
	pNode->~CNode();
	pNode->m_pNext = m_pFree;
	m_pFree = pNode;
...
template< typename E, class ETraits >
void CAtlList< E, ETraits >::RemoveHeadNoReturn() noexcept
{
	ATLENSURE( m_pHead != NULL );
...
template< typename E, class ETraits >
void CAtlList< E, ETraits >::RemoveTailNoReturn() noexcept
{
	ATLENSURE( m_pTail != NULL );
...
template< typename E, class ETraits >
void CAtlList< E, ETraits >::RemoveAt(_In_ POSITION pos) noexcept
{
	ATLASSERT_VALID(this);
	ATLENSURE( pos != NULL );
...
template< typename E, class ETraits >
POSITION CAtlList< E, ETraits >::FindIndex(_In_ size_t iElement) const noexcept
{
	ATLASSERT_VALID(this);
...
POSITION CAtlList< E, ETraits >::Find(
	/* _In_ */ INARGTYPE element,
	_In_opt_ POSITION posStartAfter) const noexcept
{
	ATLASSERT_VALID(this);
...
public:
	...
	const CPair* Lookup(/* _In_ */ KINARGTYPE key) const noexcept;
	CPair* Lookup(/* _In_ */ KINARGTYPE key) noexcept;
	...
	bool RemoveKey(/* _In_ */ KINARGTYPE key) noexcept;
	void RemoveAll();
	void RemoveAtPos(_In_ POSITION pos) noexcept;
	...
	void GetNextAssoc(
		...
		_Out_ VOUTARGTYPE value) const;
	const CPair* GetNext(_Inout_ POSITION& pos) const noexcept;
	CPair* GetNext(_Inout_ POSITION& pos) noexcept;
	...
	void GetAt(
		...
		_Out_ KOUTARGTYPE key,
		_Out_ VOUTARGTYPE value) const;
	CPair* GetAt(_In_ POSITION pos) noexcept;
	const CPair* GetAt(_In_ POSITION pos) const noexcept;
...
private:
	...
	_Ret_maybenull_ CNode* GetNode(
		...
		_Out_ UINT& iBin,
		_Out_ UINT& nHash,
		_Outref_result_maybenull_ CNode*& pPrev) const noexcept;
	CNode* CreateNode(
		/* _In_ */ KINARGTYPE key,
		_In_ UINT iBin,
		_In_ UINT nHash);
...
private:
	// Private to prevent use
	CAtlMap(_In_ const CAtlMap&) noexcept;
	CAtlMap& operator=(_In_ const CAtlMap&) noexcept;
};
...
{
public:
	CMapToInterface(_In_ UINT nBins = 17) noexcept;

private:
	// Private to prevent use
	CMapToInterface(_In_ const CMapToInterface&) noexcept;
	CMapToInterface& operator=(_In_ const CMapToInterface&) noexcept;
};

template< typename K, typename I, class KTraits >
inline CMapToInterface< K, I, KTraits >::CMapToInterface(_In_ UINT nBins) noexcept :
...
{
public:
	CMapToAutoPtr(_In_ UINT nBins = 17) noexcept;

private:
	// Private to prevent use
	CMapToAutoPtr(_In_ const CMapToAutoPtr&) noexcept;
	CMapToAutoPtr& operator=(_In_ const CMapToAutoPtr&) noexcept;
};

template< typename K, typename V, class KTraits >
inline CMapToAutoPtr< K, V, KTraits >::CMapToAutoPtr(_In_ UINT nBins) noexcept :
	CAtlMap< K, ATL::CAutoPtr< V >, KTraits, CAutoPtrElementTraits< V > >( nBins )
{
}

template< typename K, typename V, class KTraits, class VTraits >
inline size_t CAtlMap< K, V, KTraits, VTraits >::GetCount() const noexcept
{
	return( m_nElements );
}

template< typename K, typename V, class KTraits, class VTraits >
inline bool CAtlMap< K, V, KTraits, VTraits >::IsEmpty() const noexcept
{
	return( m_nElements == 0 );
}
...
template< typename K, typename V, class KTraits, class VTraits >
inline UINT CAtlMap< K, V, KTraits, VTraits >::GetHashTableSize() const noexcept
{
	return( m_nBins );
}
...
inline const typename CAtlMap< K, V, KTraits, VTraits >::CPair* CAtlMap< K, V, KTraits, VTraits >::GetAt(
	_In_ POSITION pos) const noexcept
{
	ATLASSERT( pos != NULL );
...
template< typename K, typename V, class KTraits, class VTraits >
inline void CAtlMap< K, V, KTraits, VTraits >::DisableAutoRehash() noexcept
{
	m_nLockCount++;
}

template< typename K, typename V, class KTraits, class VTraits >
inline void CAtlMap< K, V, KTraits, VTraits >::EnableAutoRehash() noexcept
{
...
template< typename K, typename V, class KTraits, class VTraits >
inline bool CAtlMap< K, V, KTraits, VTraits >::IsLocked() const noexcept
{
...
template< typename K, typename V, class KTraits, class VTraits >
UINT CAtlMap< K, V, KTraits, VTraits >::PickSize(_In_ size_t nElements) const noexcept
{
...
template< typename K, typename V, class KTraits, class VTraits >
POSITION CAtlMap< K, V, KTraits, VTraits >::GetStartPosition() const noexcept
{
	if( IsEmpty() )
	{
		return( NULL );
...
template< typename K, typename V, class KTraits, class VTraits >
void CAtlMap< K, V, KTraits, VTraits >::UpdateRehashThresholds() noexcept
{
...
template< typename K, typename V, class KTraits, class VTraits >
CAtlMap< K, V, KTraits, VTraits >::~CAtlMap() noexcept
{
	_ATLTRY
	{
		RemoveAll();
...
void CAtlMap< K, V, KTraits, VTraits >::FreeNode(_Inout_ CNode* pNode)
{
	...
	ATLASSUME( m_nElements > 0 );
	m_nElements--;
#if 0 //disable rehashing for deleting, see bug#563154
...
template< typename K, typename V, class KTraits, class VTraits >
void CAtlMap< K, V, KTraits, VTraits >::FreePlexes() noexcept
{
	m_pFree = NULL;
	if( m_pBlocks != NULL )
	{
...
const typename CAtlMap< K, V, KTraits, VTraits >::CPair* CAtlMap< K, V, KTraits, VTraits >::Lookup(
	/* _In_ */ KINARGTYPE key) const noexcept
{
	UINT iBin;
	UINT nHash;
	CNode* pNode;
...
template< typename K, typename V, class KTraits, class VTraits >
bool CAtlMap< K, V, KTraits, VTraits >::RemoveKey(/* _In_ */ KINARGTYPE key) noexcept
{
	CNode* pNode;
	UINT iBin;
	UINT nHash;
...
template< typename K, typename V, class KTraits, class VTraits >
void CAtlMap< K, V, KTraits, VTraits >::RemoveAtPos(_In_ POSITION pos) noexcept
{
	ATLENSURE( pos != NULL );
...
const typename CAtlMap< K, V, KTraits, VTraits >::CPair* CAtlMap< K, V, KTraits, VTraits >::GetNext(
	_Inout_ POSITION& pos) const noexcept
{
	CNode* pNode;
	CNode* pNext;
...
private:
	...
	// methods
	bool IsNil(_In_ CNode *p) const noexcept;
	void SetNil(_Outptr_ CNode **p) noexcept;

	CNode* NewNode(
		/* _In_ */ KINARGTYPE key,
		/* _In_ */ VINARGTYPE value);
	void FreeNode(_Inout_ CNode* pNode) noexcept;
	void RemovePostOrder(_In_ CNode* pNode) noexcept;
	CNode* LeftRotate(_In_ CNode* pNode) noexcept;
	CNode* RightRotate(_In_ CNode* pNode) noexcept;
	void SwapNode(
		_Out_ CNode* pDest,
		_Inout_ CNode* pSrc) noexcept;
	CNode* InsertImpl(
		/* _In_ */ KINARGTYPE key,
		/* _In_ */ VINARGTYPE value);
	void RBDeleteFixup(_In_ CNode* pNode) noexcept;
	bool RBDelete(_In_opt_ CNode* pZ) noexcept;
...
private:
	void VerifyIntegrity(
		...
		_Out_ int &nBlackDepth) const noexcept;
...
protected:
	CNode* Minimum(_In_opt_ CNode* pNode) const noexcept;
	CNode* Maximum(_In_opt_ CNode* pNode) const noexcept;
	CNode* Predecessor(_In_opt_ CNode* pNode) const noexcept;
	CNode* Successor(_In_opt_ CNode* pNode) const noexcept;
	CNode* RBInsert(
		/* _In_ */ KINARGTYPE key,
		/* _In_ */ VINARGTYPE value);
	CNode* Find(/* _In_ */ KINARGTYPE key) const noexcept;
	CNode* FindPrefix(/* _In_ */ KINARGTYPE key) const noexcept;

protected:
	explicit CRBTree(_In_ size_t nBlockSize = 10) noexcept;  // protected to prevent instantiation
...
public:
	...
	POSITION FindFirstKeyAfter(/* _In_ */ KINARGTYPE key) const noexcept;
	...
	const CPair* GetNext(_Inout_ POSITION& pos) const noexcept;
	CPair* GetNext(_Inout_ POSITION& pos) noexcept;
	const CPair* GetPrev(_Inout_ POSITION& pos) const noexcept;
	CPair* GetPrev(_Inout_ POSITION& pos) noexcept;
	const K& GetNextKey(_Inout_ POSITION& pos) const noexcept;
	const V& GetNextValue(_Inout_ POSITION& pos) const noexcept;
	V& GetNextValue(_Inout_ POSITION& pos) noexcept;
...
private:
	// Private to prevent use
	CRBTree(_In_ const CRBTree&) noexcept;
	CRBTree& operator=(_In_ const CRBTree&) noexcept;
};

template< typename K, typename V, class KTraits, class VTraits >
inline bool CRBTree< K, V, KTraits, VTraits >::IsNil(_In_ CNode *p) const noexcept
{
	return ( p == m_pNil );
}

template< typename K, typename V, class KTraits, class VTraits >
inline void CRBTree< K, V, KTraits, VTraits >::SetNil(_Outptr_ CNode **p) noexcept
{
	ATLENSURE( p != NULL );
	*p = m_pNil;
}
...
template< typename K, typename V, class KTraits, class VTraits >
CRBTree< K, V, KTraits, VTraits >::CRBTree(_In_ size_t nBlockSize) noexcept :
	m_pRoot( NULL ),
	m_nCount( 0 ),
	m_nBlockSize( nBlockSize ),
	m_pFree( NULL ),
...
template< typename K, typename V, class KTraits, class VTraits >
CRBTree< K, V, KTraits, VTraits >::~CRBTree() noexcept
{
	RemoveAll();
	if (m_pNil != NULL)
	{
...
template< typename K, typename V, class KTraits, class VTraits >
void CRBTree< K, V, KTraits, VTraits >::RemoveAll() noexcept
{
	if (!IsNil(m_pRoot))
...
template< typename K, typename V, class KTraits, class VTraits >
size_t CRBTree< K, V, KTraits, VTraits >::GetCount() const noexcept
{
	return m_nCount;
}

template< typename K, typename V, class KTraits, class VTraits >
bool CRBTree< K, V, KTraits, VTraits >::IsEmpty() const noexcept
{
	return( m_nCount == 0 );
}

template< typename K, typename V, class KTraits, class VTraits >
POSITION CRBTree< K, V, KTraits, VTraits >::FindFirstKeyAfter(/* _In_ */ KINARGTYPE key) const noexcept
{
	return( FindPrefix( key ) );
}

template< typename K, typename V, class KTraits, class VTraits >
void CRBTree< K, V, KTraits, VTraits >::RemoveAt(_In_ POSITION pos) noexcept
{
	ATLASSERT(pos != NULL);
...
template< typename K, typename V, class KTraits, class VTraits >
POSITION CRBTree< K, V, KTraits, VTraits >::GetHeadPosition() const noexcept
{
	return( Minimum( m_pRoot ) );
}

template< typename K, typename V, class KTraits, class VTraits >
POSITION CRBTree< K, V, KTraits, VTraits >::GetTailPosition() const noexcept
{
...
template< typename K, typename V, class KTraits, class VTraits >
void CRBTree< K, V, KTraits, VTraits >::FreeNode(_Inout_ CNode* pNode) noexcept
{
	ATLENSURE( pNode != NULL );
	pNode->~CNode();
	pNode->m_pLeft = m_pFree;
...
template< typename K, typename V, class KTraits, class VTraits >
void CRBTree< K, V, KTraits, VTraits >::RemovePostOrder(_In_ CNode* pNode) noexcept
{
	if (IsNil(pNode))
		return;
...
template< typename K, typename V, class KTraits, class VTraits >
void CRBTree< K, V, KTraits, VTraits >::SwapNode(_Out_ CNode* pDest, _Inout_ CNode* pSrc) noexcept
{
...
template< typename K, typename V, class KTraits, class VTraits >
void CRBTree< K, V, KTraits, VTraits >::RBDeleteFixup(_In_ CNode* pNode) noexcept
{
	ATLENSURE( pNode != NULL );
...
template< typename K, typename V, class KTraits, class VTraits >
bool CRBTree< K, V, KTraits, VTraits >::RBDelete(_In_opt_ CNode* pZ) noexcept
{
	if (pZ == NULL)
		return false;
...
typename CRBTree< K, V, KTraits, VTraits >::CNode* CRBTree< K, V, KTraits, VTraits >::Minimum(
	_In_opt_ CNode* pNode) const noexcept
{
...
template< typename K, typename V, class KTraits, class VTraits >
void CRBTree< K, V, KTraits, VTraits >::VerifyIntegrity() const noexcept
{
...
public:
	...
	explicit CRBMap(_In_ size_t nBlockSize = 10) noexcept;
	~CRBMap() noexcept;
...
template< typename K, typename V, class KTraits, class VTraits >
CRBMap< K, V, KTraits, VTraits >::CRBMap(_In_ size_t nBlockSize) noexcept :
	CRBTree< K, V, KTraits, VTraits >( nBlockSize )
{
}

template< typename K, typename V, class KTraits, class VTraits >
CRBMap< K, V, KTraits, VTraits >::~CRBMap() noexcept
{
}
...
template< typename K, typename V, class KTraits, class VTraits >
bool CRBMap< K, V, KTraits, VTraits >::RemoveKey(/* _In_ */ KINARGTYPE key) noexcept
{
...
public:
	...
	explicit CRBMultiMap(_In_ size_t nBlockSize = 10) noexcept;
	~CRBMultiMap() noexcept;

	POSITION Insert(
		/* _In_ */ KINARGTYPE key,
		/* _In_ */ VINARGTYPE value);
	size_t RemoveKey(/* _In_ */ KINARGTYPE key) noexcept;

	POSITION FindFirstWithKey(/* _In_ */ KINARGTYPE key) const noexcept;
...
template< typename K, typename V, class KTraits, class VTraits >
CRBMultiMap< K, V, KTraits, VTraits >::CRBMultiMap(_In_ size_t nBlockSize) noexcept :
	CRBTree< K, V, KTraits, VTraits >( nBlockSize )
{
}

template< typename K, typename V, class KTraits, class VTraits >
CRBMultiMap< K, V, KTraits, VTraits >::~CRBMultiMap() noexcept
{
}
