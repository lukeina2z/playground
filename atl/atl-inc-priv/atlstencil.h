...
// This code adapted for MSA from the Active Template Library.
#pragma once
...
#include <atlsrvres.h>
inline constexpr size_t ATL_STENCIL_MAX_ERROR_LEN = 256;
...
inline constexpr DWORD STENCIL_TEXTTAG             = 0x00000000;
inline constexpr DWORD STENCIL_REPLACEMENT         = 0x00000001;
inline constexpr DWORD STENCIL_ITERATORSTART       = 0x00000002;
inline constexpr DWORD STENCIL_ITERATOREND         = 0x00000003;
inline constexpr DWORD STENCIL_CONDITIONALSTART    = 0x00000004;
inline constexpr DWORD STENCIL_CONDITIONALELSE     = 0x00000005;
inline constexpr DWORD STENCIL_CONDITIONALEND      = 0x00000006;
inline constexpr DWORD STENCIL_STENCILINCLUDE      = 0x00000007;
inline constexpr DWORD STENCIL_STATICINCLUDE       = 0x00000008;
inline constexpr DWORD STENCIL_LOCALE              = 0x00000009;
inline constexpr DWORD STENCIL_CODEPAGE            = 0x0000000a;

// The base for user defined token types
inline constexpr DWORD STENCIL_USER_TOKEN_BASE     = 0x00001000;

// Symbols to use in error handling in the stencil processor
inline constexpr DWORD STENCIL_INVALIDINDEX = 0xFFFFFFFF;
inline constexpr DWORD STENCIL_INVALIDOFFSET = 0xFFFFFFFF;

// error codes
#define STENCIL_SUCCESS     HTTP_SUCCESS
#define STENCL_FAIL         HTTP_FAIL

inline constexpr DWORD STENCIL_BASIC_MAP = 0;
inline constexpr DWORD STENCIL_ATTR_MAP = 1;

inline constexpr size_t ATL_MAX_METHOD_NAME_LEN = 64;
inline constexpr size_t ATL_MAX_BLOCK_STACK = 128;

inline constexpr size_t k_szCurliesLen = 2;
...
struct CTagReplacerMethodsEx
{
	...
	static REPLACE_FUNC_EX_V CheckRepl(REPLACE_FUNC p) noexcept
	{
		return (REPLACE_FUNC_EX_V) p;
	}

	static REPLACE_FUNC_EX_V CheckReplEx(REPLACE_FUNC_EX p) noexcept
	{
		return (REPLACE_FUNC_EX_V) p;
	}

	static PARSE_FUNC_V CheckParse(PARSE_FUNC p) noexcept
	{
...
inline constexpr int REPLACEMENT_ENTRY_DEFAULT = 0;
inline constexpr int REPLACEMENT_ENTRY_ARGS = 1;
...
The original BEGIN_REPLACEMENT_METHOD_MAP is not thread safe. A thread-safe macro
BEGIN_THREAD_SAFE_REPLACEMENT_METHOD_MAP is added and should be used by all passport
classes.  Here is summary of the changes:

1.  Renames the old GetReplacmentMethodMap into GetReplacementMethodMapNotThreadSafe
    and wraps it around with a new thread-safe GetReplacmentMethodMap method.

2.  The Critical section is only used for the very first initialization call.

3.  Static class variables are used to implement the thread protection for each handler
    class and are declared in macro PASSPORT_HANDLER_ENTRY defined in pphandler.h that
    should be used by all passport ATL handlers.

***************************************************************************************/

#define BEGIN_THREAD_SAFE_REPLACEMENT_METHOD_MAP(className)\
public:\
    static CComAutoCriticalSection m_CritSec;\
    static bool m_bInitialized;\
    void GetReplacementMethodMap(const ATL::CTagReplacerMethodEntry<className> ** ppOut)\
    {\
        if (!m_bInitialized)\
        {\
            ...
            if (!m_bInitialized)\
            {\
                GetReplacementMethodMapNotThreadSafe(ppOut);\
                m_bInitialized = true;\
                return;\
            }\
        }\
    ...
    void GetReplacementMethodMapNotThreadSafe(const ATL::CTagReplacerMethodEntry<className> ** ppOut) const\
    {\
...
public:
		...
		DWORD *pdwMap, void **ppvParam, IAtlMemMgr *pMemMgr)
	{
		...
		*pdwMethodOffset = STENCIL_INVALIDOFFSET;
...
#ifdef DEBUG
        HTTP_CODE hcTemp= 0xdeadbeef;
                const CTagReplacerMethodEntry<T> *pEntryTemp = NULL;
#endif
        ...
        // JeffStei:  Added ASSERT here to help track pages that have invalid stencil calls in
        //            them.
        //
        //            If there is an error finding a handler call, this assert will be hit.
        //            Previously, this would result in a page returning 500, or, if
        //            DEBUG_STENCILS was enabled, printing out {{<offendinghandlername>}}
        //
        //            If this does get hit, look at szMethodName variable to see what
        //            it could not load as a handler.
        //            pEntryTemp is the methods array that it could not find.
        #ifdef DEBUG
        if (hcErr != HTTP_SUCCESS)
        {   ATLTRACE("ERROR - METHOD NAME NOT FOUND IN HANDLER: ");
            ATLTRACE(szMethodName);
            ATLTRACE("\n");
            //ATLASSERT(false);
        }
        #endif
	...
	HTTP_CODE FindReplacementOffsetInMap(
		...
		const CTagReplacerMethodEntry<T> *pEntry) noexcept
	{
		if (pEntry == NULL)
        {
			return HTTP_FAIL;
...
inline LPCSTR SkipSpace(LPCSTR sz, WORD nCodePage) noexcept
{
	if (sz == NULL)
		return NULL;
...
inline LPCSTR RSkipSpace(LPCSTR pStart, LPCSTR sz, WORD nCodePage) noexcept
{
...
private:
    ...
    long m_lTotalTokens;
    long m_lCurrentLast;
    StencilToken *m_arrTokens; //An array of tokens.

    // no use member to occupy the space
    CAtlArray<StencilToken> m_arrNoUse01; //An array of tokens.
#else
    // no use member to occupy the space
    long m_lNoUse01;
    long m_lNoUse02;
    StencilToken *m_arrNoUse03; //An array of tokens.
	...
	struct ParseError
	{
		...
		bool operator==(const ParseError& that) const noexcept
		{
	...
	public:
		...
		CParseErrorProvider() noexcept :
			m_pErrors(NULL),
			m_nCurrentError(-1)
		{
		}

		void Initialize(CSimpleArray<ParseError> *pErrors) noexcept
		{
			m_pErrors = pErrors;
		}

		HTTP_CODE OnGetNextError() noexcept
		{
			m_nCurrentError++;
...
#if 1   //passport change
            CHttpResponseOutNotSafe::PPNotSafeWrite(&c, (*m_pErrors)[m_nCurrentError].m_nLineNumber);
#else
...
#if 1   //passport change
            CHttpResponseOutNotSafe::PPNotSafeWrite(&c, static_cast<LPCSTR>((*m_pErrors)[m_nCurrentError].m_szError));
#else
	...
	HTTP_CODE LoadFromResourceInternal(HINSTANCE hInstRes, HRSRC hRsrc) noexcept
	{
...
protected:
	...
	inline BOOL CheckTag(LPCSTR szTag, DWORD dwTagLen, LPCSTR szStart, DWORD dwLen) noexcept
	{
		if (dwLen < dwTagLen)
			return FALSE;
	...
	inline void FindTagArgs(LPCSTR& szstart, LPCSTR& szend, int nKeywordChars) noexcept
	{
	...
	DWORD CheckTopAndPop(DWORD *pBlockStack, DWORD *pdwTop, DWORD dwToken) noexcept
	{
		if (*pdwTop == 0)
	...
	DWORD PushToken(DWORD *pBlockStack, DWORD *pdwTop, DWORD dwIndex) noexcept
	{
...
public:
	...
	CStencil(IAtlMemMgr *pMemMgr=NULL) noexcept
	{
		m_pBufferStart = NULL;
		m_pBufferEnd = NULL;
		m_hCacheItem = NULL;
...
#ifdef PERFTUNING
        m_arrTokens = NULL; //An array of tokens.
        m_lTotalTokens = 0;
        m_lCurrentLast = 0;
#else
	...
	bool AddErrorInternal(LPCSTR szErrorText, LPCSTR szPosition) noexcept
	{
		int nLineNum = 0;
		...
		while (szPtr < szPosition)
		{
			...
			LPCSTR szNext = CharNextExA(m_nCodePage, szPtr, 0);
			if (szNext == szPtr)
			{
				break;
			}
		...
		while (*szPtr)
		{
			...
			szEndLine = szPtr;
			LPCSTR szNext = CharNextExA(m_nCodePage, szPtr, 0);
            if ((szNext == szPtr) || (szNext >= m_pBufferEnd))
			{
				break;
			}
			szPtr = szNext;
	...
	bool AddError(UINT uID, LPCSTR szPosition) noexcept
	{
		if (m_hResInst != NULL)
		{
			_ATLTRY
	...
	bool AddReplacementError(LPCSTR szReplacement, LPCSTR szPosition) noexcept
	{
		if (m_hResInst != NULL)
		{
			_ATLTRY
	...
	void SetErrorResource(HINSTANCE) noexcept
	{
	}
	...
	{
#ifdef PERFTUNING
        int nSize = m_lCurrentLast;
#else // perftuning //TODO is this really required ?
...
#ifdef PERFTUNING
        m_pMemMgr->Free((void*)m_arrTokens);
        m_arrTokens = NULL; //An array of tokens.
        m_lTotalTokens = 0;
        m_lCurrentLast = 0;
#else
...
#ifdef PERFTUNING
            m_pMemMgr->Free((void *)m_pBufferStart);
#else
		{
	...
	bool GetHandlerName(__out_ecount_z(nPathLen) LPSTR szDllPath, __in size_t nPathLen, __out_ecount_z(nHandlerNameLen) LPSTR szHandlerName, __in size_t nHandlerNameLen) noexcept
	{
	...
	ATL_NOINLINE DWORD AddToken(
		...
		DWORD_PTR dwData = 0,
		DWORD dwMap = 0,
		BOOL bDynamicAlloc = 0) noexcept
	{
#ifdef PERFTUNING
        if (m_lCurrentLast == m_lTotalTokens)
        {
            m_arrTokens = (StencilToken *)m_pMemMgr->Reallocate(m_arrTokens,
                            (128+m_lCurrentLast)*sizeof(StencilToken));
            m_lTotalTokens += 128;
        }
        StencilToken & t = m_arrTokens[m_lCurrentLast++];
#else
		StencilToken t;
#endif
	...
	HTTP_CODE LoadFromFile(LPCSTR szFileName) noexcept
	{
		HRESULT hr = E_FAIL;
		ULONGLONG dwLen = 0;
		CAtlFile file;
		...
		_ATLTRY
		{
			...
			if (GetFileTime(file, NULL, NULL, &m_ftLastModified))
			{
				if (SUCCEEDED(file.GetSize(dwLen)))
				{
					...
					if (!buffer.Allocate(static_cast<size_t>(dwLen + 1)))
						return AtlsHttpError(500, ISE_SUBERR_OUTOFMEM); // out of memory

					DWORD dwRead = 0;
					hr = file.Read(buffer, static_cast<DWORD>(dwLen), dwRead);
					if (FAILED(hr))
                        return AtlsHttpError(500, ISE_SUBERR_READFILEFAIL); // ReadFile failed

					// some functions (e.g. SkipSpace) can read off the end of the buffer if the content
					// off the end of the buffer has a particular shape. Allocate an extra byte and set it
					// to null to make such issues less likely.
					buffer[dwRead] = '\0';
	...
	HTTP_CODE LoadFromResource(HINSTANCE hInstRes, LPCSTR szID, LPCSTR szType = NULL) noexcept
	{
		if (szType == NULL)
		{
	...
	HTTP_CODE LoadFromResourceEx(HINSTANCE hInstRes, LPCSTR szID, WORD wLanguage, LPCSTR szType = NULL) noexcept
	{
		if (szType == NULL)
		{
			szType = (LPCSTR) RT_HTML;
	...
	// loads a stencil from the specified resource
	HTTP_CODE LoadFromResource(HINSTANCE hInstRes, UINT nId, LPCSTR szType = NULL) noexcept
	{
		return LoadFromResource(hInstRes, MAKEINTRESOURCEA(nId), szType);
	}

	HTTP_CODE LoadFromResourceEx(HINSTANCE hInstRes, UINT nId, WORD wLanguage, LPCSTR szType = NULL) noexcept
	{
		return LoadFromResourceEx(hInstRes, MAKEINTRESOURCEA(nId), wLanguage, szType);
	}

	// loads a stencil from a string
	HTTP_CODE LoadFromString(LPCSTR szString, DWORD dwSize) noexcept
	{
	...
	{
#ifdef PERFTUNING
        DWORD dwSize = (DWORD) m_lCurrentLast;
#else
	...
	DWORD ParseWhile(LPCSTR szTokenStart, LPCSTR szTokenEnd, DWORD *pBlockStack, DWORD *pdwTop) noexcept
	{
	...
	DWORD ParseEndWhile(LPCSTR szTokenStart, LPCSTR szTokenEnd, DWORD *pBlockStack, DWORD *pdwTop) noexcept
	{
	...
	DWORD ParseIf(LPCSTR szTokenStart, LPCSTR szTokenEnd, DWORD *pBlockStack, DWORD *pdwTop) noexcept
	{
	...
	DWORD ParseElse(LPCSTR szTokenStart, LPCSTR szTokenEnd, DWORD *pBlockStack, DWORD *pdwTop) noexcept
	{
	...
	DWORD ParseEndIf(LPCSTR szTokenStart, LPCSTR szTokenEnd, DWORD *pBlockStack, DWORD *pdwTop) noexcept
	{
		DWORD dwTopIndex = CheckTopAndPop(pBlockStack, pdwTop, STENCIL_CONDITIONALSTART);
		if (dwTopIndex == STENCIL_INVALIDINDEX)
		{
	...
	DWORD ParseLocale(LPCSTR szTokenStart, LPCSTR szTokenEnd) noexcept
	{
		LPCSTR szstart = szTokenStart;
		LPCSTR szend = szTokenEnd;
	...
	DWORD ParseCodepage(LPCSTR szTokenStart, LPCSTR szTokenEnd) noexcept
	{
		LPCSTR szstart = szTokenStart;
		LPCSTR szend = szTokenEnd;
	...
	PARSE_TOKEN_RESULT ParseHandler(LPCSTR szTokenStart, LPCSTR szTokenEnd) noexcept
	{
		LPCSTR szstart = szTokenStart;
		LPCSTR szend = szTokenEnd;
	...
	virtual PARSE_TOKEN_RESULT ParseToken(LPCSTR szTokenStart, LPCSTR szTokenEnd, DWORD *pBlockStack, DWORD *pdwTop)
	{
		...
		ATLASSERT((szTokenEnd - szTokenStart) > k_szCurliesLen);
		pStart += k_szCurliesLen; //skip curlies
		pStart = SkipSpace(pStart, m_nCodePage);
		DWORD dwLen = (DWORD)(szTokenEnd - pStart);
	...
	virtual bool ParseReplacementsFromBuffer(ITagReplacer* pReplacer, LPCSTR pStart, LPCSTR pEnd)
	{
		...
		while(szCurr < szEnd)
		{
			...
			{
				LPCSTR szNext = CharNextExA(m_nCodePage, szCurr, 0);
				if (szNext == szCurr)
				{
					// embedded null
			...
			LPCSTR szEndTag = CharNextExA(m_nCodePage, szCurr, 0);
			if (szEndTag == szCurr)
			{
				// embedded null
				AddError(IDS_STENCIL_EMBEDDED_NULL, NULL);
	...
	HTTP_CODE GetHandlerAndMethodNames(
		...
		__in size_t nHandlerNameLen) noexcept
	{
	...
	inline BOOL IsValidIndex(DWORD dwIndex) const noexcept
	{
		if (dwIndex == STENCIL_INVALIDINDEX)
			return FALSE;
	...
	DWORD GetTokenCount() const noexcept
	{
#ifdef PERFTUNING
        return m_lCurrentLast;
#else
	...
	const StencilToken* GetToken(DWORD dwIndex) const noexcept
	{
		return &(m_arrTokens[dwIndex]);
	}

	StencilToken* GetToken(DWORD dwIndex) noexcept
	{
		return &(m_arrTokens[dwIndex]);
	}

	LPCSTR GetBufferStart() const noexcept
	{
		return m_pBufferStart;
	}

	LPCSTR GetBufferEnd() const noexcept
	{
		return m_pBufferEnd;
	}

	WORD GetCodePage() const noexcept
	{
		return m_nCodePage;
	}
...
public:
	...
	CIncludeServerContext() noexcept
	{
		m_pStream = NULL;
		m_pIncludeInfo = NULL;
	}
	...
	void Initialize(
		...
		const StencilIncludeInfo * pIncludeInfo) noexcept
	{
...
public:
	...
	BOOL Initialize(
		...
		CHttpRequest *pRequest) noexcept
	{
	...
	{
		ATLASSUME(m_pResponse != NULL);
        return m_pResponse->Redirect(pszRedirectURL, NULL);
...
public:
	static ULONG Hash( INARGTYPE pair ) noexcept
	{
		ULONG nHash = HashStr(0, pair.strDllPath);
		return HashStr(nHash, pair.strHandlerName);
	}

	static bool CompareElements( INARGTYPE pair1, INARGTYPE pair2 ) noexcept
	{
		return( (pair1.strDllPath == pair2.strDllPath) && (pair1.strHandlerName == pair2.strHandlerName) );
	}

	static int CompareElementsOrdered( INARGTYPE pair1, INARGTYPE pair2 ) noexcept
	{
...
public:
	...
	CHtmlStencil(IAtlMemMgr *pMemMgr=NULL) noexcept :
		CStencil(pMemMgr)
	{
	...
	BOOL GetIncludeInfo(LPCSTR szParamBegin, LPCSTR szParamEnd, StencilIncludeInfo *pInfo) const
	{
		...
		{
			LPCSTR szNext = CharNextExA(GetCodePage(), szQueryBegin, 0);
	...
	DWORD ParseInclude(LPCSTR szTokenStart, LPCSTR szTokenEnd)
	{
		...
		else
		{
			...
			if (!szBufferStart.Allocate(static_cast<size_t>(dwLen + 1)))
			{
				AddError(IDS_STENCIL_OUTOFMEMORY, szTokenStart);
				return AddToken(szTokenStart, szTokenEnd, STENCIL_TEXTTAG);
			}
			...
			DWORD dwRead{};
			if (FAILED(file.Read(szBufferStart, static_cast<DWORD>(dwLen), dwRead)))
			{
				return AddToken(szTokenStart, szTokenEnd, STENCIL_TEXTTAG);
			}

			szBufferStart[dwRead] = '\0';
	...
	PARSE_TOKEN_RESULT ParseSubhandler(LPCSTR szTokenStart, LPCSTR szTokenEnd) noexcept
	{
	...
	mapType* GetExtraHandlers() noexcept
	{
...
{
    class CReleaseStencilCache
    {
        StencilType *m_pLoadedStencil;
        IStencilCache *m_pStencilCache;
        HTTP_CODE &m_hcErr;
    public:
        CReleaseStencilCache(StencilType *pLoadedStencil, HTTP_CODE &hcErr,
            IStencilCache *pStencilCache) : m_hcErr(hcErr)
        {
            m_pLoadedStencil = pLoadedStencil;
            m_pStencilCache = pStencilCache;
        }

        ~CReleaseStencilCache()
        {
            if (m_pStencilCache && m_pLoadedStencil && !IsAsyncStatus(m_hcErr) && m_pLoadedStencil->GetCacheItem())
                m_pStencilCache->ReleaseStencil(m_pLoadedStencil->GetCacheItem());
        }
    };
...
public:
	...
	{
        HTTP_CODE hcErr = HTTP_FAIL;
        CReleaseStencilCache stencil(m_pLoadedStencil, hcErr, m_spStencilCache);
...
#if 0 // moved into the smart class to be safe from exception out of Render() above
...
//Implementation
	...
	StencilType* GetNewCacheStencil()
	{
        ...
        if (NULL != pMemMgr)
        {
            LPVOID pv = pMemMgr->Allocate(sizeof(StencilType));
            pStencil = new (pv) StencilType(pMemMgr);
        }
        else
#endif
	...
	HTTP_CODE CacheStencil(
		LPCSTR szName,
		StencilType* pStencilData) noexcept
	{
		THandler *pT = static_cast<THandler *>(this);
		HRESULT hr = E_FAIL;
	...
	StencilType *FindCacheStencil(LPCSTR szName) noexcept
	{
...
public:
    ...
    CHttpResponseOutNotSafe m_HttpResponseOutNotSafe;
	ATLSRV_REQUESTTYPE m_dwRequestType;
	AtlServerRequest* m_pRequestInfo;

	CRequestHandlerT() noexcept
	{
		m_hInstHandler = NULL;
		m_dwAsyncFlags = 0;
		m_pRequestInfo = NULL;
    ...
    // Allows a user to process any content before the buffer is flushed
    virtual void ProcessResponseContent(LPCSTR, HTTP_CODE = HTTP_SUCCESS)
    {
        // Nothing to do
    }
		...
		IServiceProvider* /*pServiceProvider*/)
	{
		...
		if (hcErr == HTTP_SUCCESS && m_pLoadedStencil)
		{
            ...
            // Allow users to do something with the rendered content
            pT->ProcessResponseContent((LPCSTR)m_HttpResponse.m_strContent, hcErr);
