...
inline CAtlFileMappingBase* CPerfMon::_OpenNextBlock(CAtlFileMappingBase* pPrev) noexcept
{
	...
	else
	{
		...
		SYSTEM_INFO si;
		GetSystemInfo(&si);
		m_nAllocSize = si.dwAllocationGranularity * 2;
...
#if 0 //remove the restriction, see bug#188486
		else
		{
			CSid owner;
			CDacl dacl;
...
inline DWORD& CPerfMon::_GetBlockId(CAtlFileMappingBase* pBlock) noexcept
{
	ATLENSURE(pBlock != NULL);
	...
	) noexcept
{
	...
	else
	{
        ...
        /*
         ...
         The PERF_INSTANCE_DEFINITION structure is used only when the performance object
         has instances. The NameLength should be set to the length of the instance name
         plus all padding bytes.
         The fixed portion of the PERF_INSTANCE_DEFINITION structure is aligned on an 8-byte boundary.
         If the instance name immediately follows this structure, then the member
         ByteLength must be set to the length of the fixed portion of the structure
         plus the NameLength (the NameLength member includes the padding bytes).
        */
		LPCWSTR szInstNameSrc = LPCWSTR(LPBYTE(pInstance)+pInstance->m_nInstanceNameOffset);
        ULONG ulInstNameLen = (ULONG)(wcslen(szInstNameSrc)+1)*sizeof(WCHAR);
        //allignup to 8 bytes
        ULONG ulNameLen = AtlAlignUp(ulInstNameLen, 8);

        LPWSTR szInstNameDest = (LPWSTR) _AllocData(pData, nBytesAvail, pnBytesUsed, ulNameLen);
		if (szInstNameDest == NULL)
			return E_OUTOFMEMORY;

        memcpy(szInstNameDest, szInstNameSrc, ulInstNameLen);
		pInstanceDef->NameOffset = ULONG(LPBYTE(szInstNameDest) - LPBYTE(pInstanceDef));

        pInstanceDef->NameLength = ulInstNameLen;
        pInstanceDef->ByteLength = DWORD(sizeof(PERF_INSTANCE_DEFINITION) + ulNameLen);
	}
	...
	) noexcept
{
    ...
    bool bAllInstancesChecked = false;
    __int64 nBytesInBlockChecked = 0;
    ...
    if (pCurrentBlock != NULL)
    {
        ...
        while (pInstance && !bAllInstancesChecked)
        {
            bool bGotoNextBlock = false;
            nBytesInBlockChecked = (LPBYTE)pInstance - (LPBYTE)pCurrentBlock + pInstance->m_nAllocSize;
            if (pInstance->m_dwCategoryId == pCategoryInfo->m_dwCategoryId)
            {
                bGotInstance = true;
            ...
            if(sizeof(CPerfObject) <= (m_nAllocSize - nBytesInBlockChecked))
            {
                //it's safe to move instance pointer
             ...
             {
                //not enough for an instance for sure, don't move instance pointer in the same block, 
                 //goto the next block
                bGotoNextBlock = true;                 
             }
            if (bGotoNextBlock)
            {
                ...
                {
                    pInstance = NULL;
                    bAllInstancesChecked = true;
                }    
                else
                {
                    pInstance = _GetFirstInstance(pCurrentBlock);
                    nBytesInBlockChecked = 0;
                }                    
            }
        }
    }
	...
	) noexcept
{
	_ATLTRY
	{
		...
		{
			nNewCounterSize += astrCounters[i].GetLength() + MAX_INT_DIGIT;
			nNewHelpSize += astrHelp[i].GetLength() + MAX_INT_DIGIT;
		}
...
inline HRESULT CPerfMon::Initialize() noexcept
{
	...
	_ATLTRY
	{
        ...
        dacl.AddAllowedAce(CSid(Sids::SecurityNTAuthority, 1, SECURITY_NETWORK_SERVICE_RID), GENERIC_ALL);
		...
		strMutexName.Format(_T("Global\\ATLPERF_%s_LOCK2"), strAppName.GetString());
...
inline HRESULT CPerfMon::_CreateInstance(
	...
	bool bByName,
    DWORD dwNumTotalInstances
	) noexcept
{
	CPerfObject* pEmptyBlock = NULL;
    DWORD dwCountInstances = 0;
	...
	*ppInstance = NULL;
	...
	while (pInstance->m_nAllocSize != 0)
	{
        ...
        if (m_nHeaderSize + nUsedSpace + pCategoryInfo->m_nAllocSize + sizeof(CPerfObject) <= m_nAllocSize)
        {
		pInstance = _GetNextInstance(pInstance);
        }
        else
        ...
        if (dwNumTotalInstances > 0 && dwCountInstances >= dwNumTotalInstances)
        {
            return E_FAIL;
        }
        if (!fGetBlock && pInstance->m_nAllocSize == 0 &&
        ...
        if (fGetBlock || pInstance->m_nAllocSize == (ULONG) -1)
		{
            fGetBlock = FALSE;
...
inline HRESULT CPerfMon::CreateInstanceByNameWithBounds(
	DWORD dwCategoryId,
	LPCWSTR szInstanceName,
	CPerfObject** ppInstance,
    DWORD dwNumTotalInstances
...
{
	return _CreateInstance(dwCategoryId, 0, szInstanceName, ppInstance, true,dwNumTotalInstances);
}
		...
		while (pInstance && pInstance->m_nAllocSize)
		{
			if (pInstance->m_dwCategoryId == pCategoryInfo->m_dwCategoryId)
			{
				if (pCategoryInfo->m_nInstanceLess != PERF_NO_INSTANCES)
				{
					...
					strXML.Format("\t\t\t<instance name=\"%s\" id=\"%d\">\r\n", szUTF8.m_pData, pInstance->m_dwInstance);
					hr = pStream->Write(strXML, strXML.GetLength(), &nLen);
					if (hr != S_OK)
						return hr;
				}
