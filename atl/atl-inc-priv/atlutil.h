...
inline BOOL IsFullPath(_In_reads_(3) LPCTSTR szPath) noexcept
{
	return AtlIsFullPathT(szPath);
}

inline BOOL IsFullPathA(_In_reads_(3) LPCSTR szPath) noexcept
{
	return AtlIsFullPathT(szPath);
}
...
inline bool SafeStringCopy(
	...
	_In_z_ const char* Source) noexcept
{
...
#define ATL_URL_MAX_PATH_LENGTH        12000
#define ATL_URL_MAX_SCHEME_LENGTH      256          // longest protocol name length
...
public:
	...
	BOOL Alloc(__in DWORD dwSize) noexcept
	{
		...
		if (m_pBuffer)
		{
            ...
            // I think we should be calling Empty(); here to set m_dwLen to 0
            // but out of scope
            //
			return TRUE;
		}
        ...
        {
            // If allocation failed, set back to fixed stack memory, don't leave as null
            // as other calls to this class check m_pBuffer != m_szBuffer means
            // we have valid memory on the heap. Still retrun FALSE indicating request
            // has failed, but leave class in a consistent state so further calls
            // don't crash
            //
            m_dwAlloc = dwSizeT;
            m_pBuffer = m_szBuffer;
...
public:
	...
	_AtlThreadContextInfo(_In_ IStackDumpHandler* p) noexcept
	{
		hThread = NULL;
	...
	~_AtlThreadContextInfo() noexcept
	{
		if (hThread != NULL)
			CloseHandle(hThread);
	}
...
public:
	CDebugReportHook(
		...
		_In_ DWORD dwTimeout = 20000) noexcept
	{
	...
	BOOL SetPipeName(
		_In_z_ LPCSTR szMachineName = ".",
		_In_z_ LPCSTR szPipeName = "AtlsDbgPipe") noexcept
	{
	...
	static int __cdecl CDebugReportHookProc(
		_In_ int reportType,
		_In_z_ char *message,
		_Inout_ int *returnValue) noexcept
	{
...
public:
	...
	_Check_return_ HRESULT Initialize(
		...
		_In_ int nNumThreads=0,
                _In_ int nNumThreadsPerProc=0,
		_In_ DWORD dwStackSize=0,
		_In_ HANDLE hCompletion=INVALID_HANDLE_VALUE) noexcept
	{
        ...
        m_nNumThreadsPerProc = nNumThreadsPerProc;
	...
	void Shutdown(_In_ DWORD dwMaxWait=0) noexcept
	{
...
ATLPREFAST_UNSUPPRESS()
        ...
        //
        // This assert has been removed. There are known issues where this triggers during
        // shutdown.
        //
	...
	_Check_return_ HRESULT STDMETHODCALLTYPE SetSize(_In_ int nNumThreads) noexcept
	{
		if (nNumThreads == 0)
        {
        if (m_nNumThreadsPerProc == 0)
			nNumThreads = -ATLS_DEFAULT_THREADSPERPROC;
        else
            nNumThreads = -m_nNumThreadsPerProc;
        }
	...
	_Check_return_ HRESULT STDMETHODCALLTYPE GetSize(_Out_ int *pnNumThreads) noexcept
	{
		if (!pnNumThreads)
			return E_POINTER;
	...
	_Check_return_ HRESULT STDMETHODCALLTYPE SetTimeout(_In_ DWORD dwMaxWait) noexcept
	{
	...
	_Check_return_ HRESULT STDMETHODCALLTYPE GetTimeout(_Out_ DWORD *pdwMaxWait) noexcept
	{
		if (!pdwMaxWait)
			return E_POINTER;
	...
	HANDLE GetQueueHandle() noexcept
	{
	...
	// in WorkerThreadProc
	BOOL QueueRequest(_In_ typename Worker::RequestType request) noexcept
	{
		ATLASSUME( m_hRequestQueue != NULL );
...
protected:
	...
	static DWORD WINAPI WorkerThreadProc(_In_ LPVOID pv) noexcept
	{
		CThreadPool* pThis =
...
public:
	...
	void Terminate(_In_opt_ void* /*pvParam*/) noexcept
	{
	}
}; // class CNonStatelessWorker
...
inline short AtlHexValue(_In_ char chIn) noexcept
{
...
inline BOOL AtlIsUnsafeUrlChar(_In_ char chIn, __in bool fIncludeSingleQuote = false) noexcept
{
	unsigned char ch = (unsigned char)chIn;

	if (ch == '\'' && fIncludeSingleQuote)
	{
		return TRUE;
	}
...
inline ATL_URL_PORT AtlGetDefaultUrlPort(_In_ ATL_URL_SCHEME m_nScheme) noexcept
{
	switch (m_nScheme)
	{
...
public:
	...
	CUrl(_In_ const CUrl& urlThat) noexcept
	{
		CopyFields(urlThat);
	}
	...
	CUrl& operator=(_In_ const CUrl& urlThat)  noexcept
	{
		CopyFields(urlThat);
		return (*this);
	}
...
ATLPREFAST_SUPPRESS(6103)
	...
	inline BOOL CreateUrl(
		...
		_Inout_ DWORD* pdwMaxLength,
		_In_ DWORD dwFlags = 0) const noexcept
	{
		ATLASSERT(lpszUrl != NULL);
...
ATLPREFAST_UNSUPPRESS()
	...
	inline DWORD GetUrlLength() const noexcept
	{
...
ATLPREFAST_UNSUPPRESS()
	...
	inline LPCTSTR GetSchemeName() const noexcept
	{
		return m_szScheme;
	}

	//Get the Scheme Name length
	inline DWORD GetSchemeNameLength() const noexcept
	{
	...
	inline ATL_URL_SCHEME GetScheme() const noexcept
	{
		return m_nScheme;
	}

	//Get the host name
	inline LPCTSTR GetHostName() const noexcept
	{
		return m_szHostName;
	}

	//Get the host name's length
	inline DWORD GetHostNameLength() const noexcept
	{
		return m_dwHostNameLength;
	}

	//Set the Host name
	inline BOOL SetHostName(_In_reads_z_(ATL_URL_MAX_HOST_NAME_LENGTH+1) LPCTSTR lpszHost) noexcept
	{
	...
	inline ATL_URL_PORT GetPortNumber() const noexcept
	{
		return m_nPortNumber;
	}

	//Set the port number in terms of ATL_URL_PORT
	inline BOOL SetPortNumber(_In_ ATL_URL_PORT nPrt) noexcept
	{
		m_nPortNumber = nPrt;
		return TRUE;
	}
	...
	//Get the user name
	inline LPCTSTR GetUserName() const noexcept
	{
		return m_szUserName;
	}

	//Get the user name's length
	inline DWORD GetUserNameLength() const noexcept
	{
		return m_dwUserNameLength;
	}

	//Set the user name
	inline BOOL SetUserName(_In_reads_z_(ATL_URL_MAX_USER_NAME_LENGTH+1) LPCTSTR lpszUser) noexcept
	{
	...
	//Get the password
	inline LPCTSTR GetPassword() const noexcept
	{
		return m_szPassword;
	}

	//Get the password's length
	inline DWORD GetPasswordLength() const noexcept
	{
	...
	//before extra info)
	inline LPCTSTR GetUrlPath() const noexcept
	{
		return m_szUrlPath;
	}

	//Get the url path's length
	inline DWORD GetUrlPathLength() const noexcept
	{
		return m_dwUrlPathLength;
	}

	//Set the url path
	inline BOOL SetUrlPath(_In_reads_z_(ATL_URL_MAX_PATH_LENGTH+1) LPCTSTR lpszPath) noexcept
	{
	...
	inline LPCTSTR GetExtraInfo() const noexcept
	{
		return m_szExtraInfo;
	}

	//Get extra info's length
	inline DWORD GetExtraInfoLength() const noexcept
	{
		return m_dwExtraInfoLength;
	}

	//Set extra info
	inline BOOL SetExtraInfo(_In_reads_z_(ATL_URL_MAX_PATH_LENGTH+1) LPCTSTR lpszInfo) noexcept
	{
	...
	inline BOOL Canonicalize(_In_ DWORD dwFlags = 0) noexcept
	{
...
private:
	...
	const _schemeinfo * GetSchemes() noexcept
	{
	...
	inline BOOL Parse(_In_z_ LPCTSTR lpszUrl)
	{
		...
		*pszCurrentUrl = '\0';
        ...
        // validate hostname (not part of standard ATL)
        // RFC 1034 does not state the underscore char is part of the domain https://datatracker.ietf.org/doc/html/rfc1034#section-3.5
        // But Graph already supports it, so adding parity in MSA.
        for (ULONG lPos=0; lPos < m_dwHostNameLength; lPos++)
        {
            if(!(m_szHostName[lPos] == '.' || m_szHostName[lPos] == ':' ||
                 m_szHostName[lPos] == '-' || m_szHostName[lPos] == '_' ||
                 (m_szHostName[lPos] == '[' && lPos == 0) ||
                 (m_szHostName[lPos] == ']' && lPos == m_dwHostNameLength - 1) ||
                 (m_szHostName[lPos] >= '0' && m_szHostName[lPos] <= '9') ||
                 (m_szHostName[lPos] >= 'A' && m_szHostName[lPos] <= 'Z') ||
                 (m_szHostName[lPos] >= 'a' && m_szHostName[lPos] <= 'z')))
            {
                goto error;
            }
...
error:
	...
	ATL_NOINLINE void InitFields() noexcept
	{
		m_nPortNumber = ATL_URL_INVALID_PORT_NUMBER;
		m_nScheme = ATL_URL_SCHEME_UNKNOWN;
	...
	inline void CopyFields(_In_ const CUrl& urlThat) noexcept
	{
...
#ifdef PASSPORT_PAGEHEAP_RUN
#define ATL_WORKER_THREAD_WAIT 100000	// time to wait when shutting down
#endif
...
protected:
	...
	_Check_return_ HRESULT Refresh() noexcept
	{
...
public:
	...
	HANDLE GetThreadHandle() noexcept
	{
		if (m_pThread)
	...
	_Check_return_ HRESULT Initialize() noexcept
	{
		if (m_pThread)
	...
	_Check_return_ HRESULT Initialize(_In_ CWorkerThread<ThreadTraits> *pThread) noexcept
	{
		if (!pThread)
			return E_INVALIDARG;
	...
	_Check_return_ HRESULT AddHandle(
		...
		_In_ DWORD_PTR dwParam) noexcept
	{
		if (m_pThread)
	...
	_Check_return_ HRESULT RemoveHandle(_In_ HANDLE hObject) noexcept
	{
		if (m_pThread)
	...
	_Check_return_ HRESULT Shutdown(_In_ DWORD dwWait=ATL_WORKER_THREAD_WAIT) noexcept
	{
		if (m_pThread)
			return S_OK;
...
private:
	...
	int CopyHandles(_Out_writes_to_(MAXIMUM_WAIT_OBJECTS, return) HANDLE handles[MAXIMUM_WAIT_OBJECTS]) noexcept
	{
	...
	int CopyClientEntries(
		_Out_writes_to_(MAXIMUM_WAIT_OBJECTS, return) WorkerClientEntry clientEntries[MAXIMUM_WAIT_OBJECTS]) noexcept
	{
...
#pragma warning(disable: 4702) // Unreachable code.
	...
	static DWORD WINAPI _WorkerThreadProc(_Inout_ CWorkerThread *pThis) noexcept
	{
		_ATLTRY
		{
...
public:
	...
	HRESULT AddTimer(
		...
		_In_opt_ IWorkerThreadClient * /*pClient*/,
		_In_ DWORD_PTR /*dwParam*/,
		_In_opt_ HANDLE * /*phTimer*/) noexcept
	{
		return S_OK;
	}

	HRESULT RemoveHandle(_In_ HANDLE /*hObject*/) noexcept
	{
		return S_OK;
	}

	HRESULT Shutdown(_In_ DWORD dwWait=ATL_WORKER_THREAD_WAIT) noexcept
	{
		(dwWait);
		return S_OK;
	}
...
inline BOOL CopyCString(
	...
	_Inout_ DWORD *pdwDestLen) noexcept
{
	if (!pdwDestLen)
		return FALSE;
