...
{
public:
	CAtlNavigateData() noexcept; // public construction
	...
	DWORD SetFlags(DWORD dwNewFlags) noexcept; // set all flags
	DWORD GetFlags() noexcept; // get value of flags
	DWORD AddFlags(DWORD dwFlagsToAdd) noexcept; // add one or more flags to existing flags
	DWORD RemoveFlags(DWORD dwFlagsToRemove) noexcept; // remove one or more flags from existing flags
	LPCTSTR SetExtraHeaders(LPCTSTR szNewHeaders) noexcept; // set the extra request headers
	LPCTSTR GetExtraHeaders() noexcept; // get the extra request headers
	LPCTSTR SetMethod(LPCTSTR szNewMethod) noexcept; // set the HTTP request method
	LPCTSTR GetMethod() noexcept; // get the HTTP request method
	short SetPort(short newPort) noexcept; // set the TCP port for this request
	short GetPort() noexcept; // get the TCP port for this request
	void SetPostData(BYTE *pData, DWORD dwDataLen, LPCTSTR szDataType) noexcept; // Set data to be sent as the reqeust entity body
	DWORD SetSocketTimeout(DWORD dwNewTimeout) noexcept; // Set the timeout for this socket
	DWORD GetSocketTimeout() noexcept; // Get the timeout for this socket
	DWORD SetSendBlockSize(DWORD dwBlockSize) noexcept; // Set the size of the blocks used to send data
	DWORD GetSendBlockSize() noexcept; // get the size of the blocks used to send data
	DWORD SetReadBlockSize(DWORD dwBlockSize) noexcept; // Set the size of the blocks used to send data
	DWORD GetReadBlockSize() noexcept; // get the size of the blocks used to send data
	PFNATLCHUNKEDCB SetChunkCallback(PFNATLCHUNKEDCB pfn, DWORD_PTR dwParam) noexcept; // set the callback function used for sending chunked data
	PFNATLCHUNKEDCB GetChunkCallback() noexcept; // get the chunked callback function
	PFNATLSTATUSCALLBACK SetSendStatusCallback(PFNATLSTATUSCALLBACK pfn, DWORD_PTR dwData) noexcept; // sets a function pointer to be called after bytes are sent over the socket
	PFNATLSTATUSCALLBACK GetSendStatusCallback() noexcept; // returns current status callback function
	PFNATLSTATUSCALLBACK SetReadStatusCallback(PFNATLSTATUSCALLBACK pfn, DWORD_PTR dwData) noexcept;
	PFNATLSTATUSCALLBACK GetReadStatusCallback() noexcept;
};
...
public:
	...
	bool SetProxy(LPCTSTR szProxy = NULL, short nProxyPort = 0) noexcept;
	void RemoveProxy() noexcept;
	LPCTSTR GetProxy() const noexcept;
	short GetProxyPort() const noexcept;
	...
	bool AddAuthObj(LPCTSTR szScheme, CAtlBaseAuthObject *pObject, IAuthInfo *pInfo=NULL) noexcept;
	const CAtlBaseAuthObject* FindAuthObject(LPCTSTR szScheme) noexcept;
	bool RemoveAuthObject(LPCTSTR szScheme) noexcept;
	virtual bool NegotiateAuth(bool bProxy) noexcept;


	// Retrieve the value of a response header
	bool GetHeaderValue(LPCTSTR szName, CString& strValue) const noexcept; 
	bool GetHeaderValue(__in_z LPCTSTR szName, __out_ecount_part_z_opt(*pdwLen, *pdwLen) LPTSTR szBuffer, __inout DWORD *pdwLen) const noexcept;

	DWORD GetResponseLength() noexcept; // Get the number of bytes in the response
	const BYTE* GetResponse() noexcept; // Get the entire response
	DWORD GetBodyLength() const noexcept; // Get the length of the body of the response (everything after the \r\n\r\n)
	const BYTE* GetBody() noexcept; // Get the body of the response (length is determined by GetBodyLength())
	DWORD GetRawResponseHeaderLength() noexcept; // Get the length of the raw request headers
	bool GetRawResponseHeader(LPBYTE szBuffer, DWORD *pdwLen) noexcept; // Get the raw request headers
	LPCURL GetCurrentUrl() const noexcept; // Get a pointer to the current URL for this request
	DWORD GetFlags() const noexcept; // Retrieve flags used for processing this request
	int GetStatus() noexcept; // Get the HTTP status code that resulted from making this request
	LPCTSTR GetMethod() noexcept; // Get the HTTP method used for making this request
	BYTE* GetPostData() noexcept; // Get a pointer to raw data being sent with this request
	DWORD GetPostDataLen() noexcept; // Get the length of the raw data sent with this request
	LPCTSTR GetPostDataType() noexcept; // Get the data type (sent as Content-Type header) for this request
	DWORD GetLastError() noexcept; // Retrieves errors from the underlying socket
	const SOCKET& GetSocket() noexcept; // Retrieves the underlying socket. Be careful!
	void Close() noexcept; // Close the connection
	DWORD SetSocketTimeout(DWORD dwNewTimeout) noexcept; // Sets a new socket timeout, returns the old timeout.
	DWORD GetSocketTimeout() noexcept; // retrieves the current socket timeout
	void AuthProtocolFailed(LPCTSTR szProto) noexcept; // notifies us of failure to connect with the named protocol
...
// Implementation
	HTTP_RESPONSE_READ_STATUS ReadHttpResponse() noexcept;
	void ResetConnection() noexcept;
	bool ProcessStatus(DWORD dwFlags) noexcept;
	bool BuildRequest(/*out*/CString *pstrRequest,
						LPCTSTR szDataType=NULL,
						LPCTSTR szExtraHeaders=NULL) noexcept;
...
protected:
	...
	bool SetDefaultUrl(LPCTSTR szUrl, short nPortNumber=ATL_URL_DEFAULT_HTTP_PORT) noexcept;
	bool SetDefaultUrl(LPCURL pUrl, short nPortNumber=ATL_URL_DEFAULT_HTTP_PORT) noexcept;
	bool SetDefaultMethod(LPCTSTR szMethod) noexcept;
	void InitializeObject() noexcept;
	void ResetRequest() noexcept;
	bool ReadSocket() noexcept;
	unsigned char* FindHeaderEnd(unsigned char** ppBegin) noexcept;
	bool LookupRegProxy() noexcept;
	bool DisconnectIfRequired() noexcept;
	bool ConnectSocket() noexcept;

	long GetContentLength() noexcept;
	LPCSTR NextLine(BYTE* pCurr) noexcept;
	bool IsMsgBodyChunked() noexcept;
	LPCSTR FindEndOfHeader(LPCSTR pszStart) noexcept;
	bool DecodeHeader(LPCSTR pHeaderStart, LPCSTR pHeaderEnd) noexcept;
	virtual void OnSetCookie(LPCTSTR /*szCookie*/) noexcept;
	LPCSTR ParseStatusLine(BYTE* pBuffer) noexcept;
	int CrackResponseHeader(LPCSTR pBuffer, /*out*/ LPCSTR *pEnd) noexcept;
	bool ReadBody(int nContentLen, int nCurrentBodyLen) noexcept;
	bool ReadChunkedBody() noexcept;
	bool ReconnectIfRequired() noexcept;
	bool CompleteURL(CString& strURL) noexcept;
	bool ProcessObjectMoved() noexcept;
	bool _SetDefaultUrl(LPCTSTR szURL, short nPort) noexcept;
	...
	CHUNK_LEX_RESULT get_chunked_size(__deref_inout char *&pBuffStart, __deref_inout char *&pBuffEnd, __inout long* pnChunkSize) noexcept;
	bool move_leftover_bytes(__in_ecount(nLen) char *pBufferStart, __in int nLen, __deref_inout char *&pBuffStart, __deref_inout char *&pBuffEnd) noexcept;
	CHUNK_LEX_RESULT get_chunked_data(__deref_inout char *&pBufferStart, __deref_inout char *&pBufferEnd, long nChunkSize,
								  __deref_out_ecount_part(*pnDataLen, *pnDataLen) char **ppDataStart, __inout long *pnDataLen) noexcept;
	CHUNK_LEX_RESULT consume_chunk_trailer(__deref_inout char *&pBufferStart, __deref_inout char *pBufferEnd) noexcept;
	CHUNK_LEX_RESULT consume_chunk_footer(__deref_inout char *&pBufferStart, __deref_inout char *&pBufferEnd) noexcept;
...
{
public:
	virtual ~CNTLMAuthObject() noexcept;
	CNTLMAuthObject() noexcept;
	CNTLMAuthObject(IAuthInfo *pAuthInfo) noexcept;
	void SetAuthInfo(IAuthInfo *pAuthInfo) noexcept;
	bool GetCredentialNames(CString& theName);

// Implementation
	// Called by the CAtlHttpClient class to authenticate a user.
	virtual void Init(CAtlHttpClient *pSocket, IAuthInfo *pAuthInfo=NULL) noexcept;

	// Called by the CAtlHttpClient class to initialize this authentication object.
	virtual bool Authenticate(LPCTSTR szAuthTypes, bool bProxy) noexcept;
protected:
	bool AcquireCredHandle() noexcept;
	// This function creates an NTML Authorization header
	// and sends it to the HTTP server.
	bool SendSecurityInfo(SecBuffer *pSecBuffer, LPSTR *pszBuffer) noexcept;
	bool DoNTLMAuthenticate() noexcept;
...
{
public:
	CBasicAuthObject() noexcept;
	CBasicAuthObject(IAuthInfo *pAuthInfo) noexcept;
	void SetAuthInfo(IAuthInfo *pAuthInfo) noexcept;
	LPCTSTR GetRealm() noexcept; // Retrieve's the realm being used.
...
// Implementation
	...
	virtual void Init(CAtlHttpClient *pSocket, IAuthInfo *pAuthInfo=NULL) noexcept;
protected:
	bool DoBasicAuthenticate() noexcept;
	bool CrackRealm(LPCTSTR szHeader) noexcept;
...
inline bool _AtlGetAuthInfoHelper(IAuthInfo *pObj, PFNAUTHFUNC pFunc, CAuthInfoBuffType& buff, DWORD *dwLen) noexcept
{
...
public:
	...
	bool Init(IAuthInfo *pAuthInfo) noexcept
	{
		if (!pAuthInfo)
			return false;
...
public:
	...
	bool SetSize(unsigned long nSize) noexcept
	{
		if (!nSize)
			return false;
	...
	bool ClearBuffer(unsigned long nSize) noexcept
	{
		if(nSize > m_cbAlloc)
			return false;
...
public:
	...
	// index is 0 based
	CSecBuffer* Buffers(unsigned int i) noexcept
	{
		if (i < cBuffers)
		{
	...
	bool AddBuffers(unsigned int nCount, unsigned int nBufferSize) noexcept
	{
		if (!nCount)
			return true;
	...
	operator PSecBufferDesc() noexcept
	{
