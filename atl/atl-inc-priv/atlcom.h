...
// Construction
public:
   CComTypeAttr(_In_ ITypeInfo* pTypeInfo) noexcept :
	  m_pTypeAttr( NULL ),
	  m_pTypeInfo( pTypeInfo )
   {
   }
   ~CComTypeAttr() noexcept
   {
	  Release();
   }
...
public:
   ...
   operator const TYPEATTR*() const noexcept
   {
	  return m_pTypeAttr;
   }
...
// Construction
public:
   CComVarDesc(_In_ ITypeInfo* pTypeInfo) noexcept :
	  m_pVarDesc( NULL ),
	  m_pTypeInfo( pTypeInfo )
   {
   }
   ~CComVarDesc() noexcept
   {
	  Release();
   }
...
public:
   ...
   operator const VARDESC*() const noexcept
   {
	  return m_pVarDesc;
   }
...
// Construction
public:
   CComFuncDesc(_In_ ITypeInfo* pTypeInfo) noexcept :
	  m_pFuncDesc( NULL ),
	  m_pTypeInfo( pTypeInfo )
   {
   }
   ~CComFuncDesc() noexcept
   {
	  Release();
   }
...
public:
   ...
   operator const FUNCDESC*() const noexcept
   {
	  return m_pFuncDesc;
   }
...
#else
#define _ATL_DECLARE_GET_UNKNOWN(x) IUnknown* GetUnknown() noexcept {return _GetRawUnknown();}
#endif
...
#define BEGIN_COM_MAP(x) public: \
	...
	static HRESULT WINAPI _Cache(_In_ void* pv, _In_ REFIID iid, _COM_Outptr_result_maybenull_ void** ppvObject, _In_ DWORD_PTR dw) noexcept\
	{\
		_ComMapClass* p = (_ComMapClass*)pv;\
		p->Lock();\
		HRESULT hRes = EVENT_E_INTERNALEXCEPTION; \
		__try \
		{ \
	...
	IUnknown* _GetRawUnknown() noexcept \
	...
	const static ATL::_ATL_INTMAP_ENTRY* WINAPI _GetEntries() noexcept { \
...
#define BEGIN_ATTRCOM_MAP(x) public: \
	...
	const static ATL::_ATL_INTMAP_ENTRY* WINAPI _GetAttrEntries() noexcept { \
	static const ATL::_ATL_INTMAP_ENTRY _entries[] = {

#define DECLARE_GET_CONTROLLING_UNKNOWN() public:\
	virtual IUnknown* GetControllingUnknown() noexcept {return GetUnknown();}
...
#define BEGIN_CATEGORY_MAP(x)\
	static const struct ATL::_ATL_CATMAP_ENTRY* GetCategoryMap() noexcept {\
...
#define DECLARE_NO_REGISTRY()\
	static HRESULT WINAPI UpdateRegistry(_In_ BOOL /*bRegister*/) noexcept\
	{return S_OK;}

#ifdef _ATL_USE_WINAPI_FAMILY_DESKTOP_APP

#define DECLARE_OBJECT_DESCRIPTION(x)\
	static LPCTSTR WINAPI GetObjectDescription() noexcept\
	{\
		return _T(x);\
	}
...
#define DECLARE_OLEMISC_STATUS(x) \
	static DWORD _GetMiscStatus() noexcept \
	{ \
...
public:
	...
	HRESULT STDMETHODCALLTYPE QueryInterface(
		_COM_Outptr_ Q** pp) noexcept
	{
		return QueryInterface(__uuidof(Q), (void**)pp);
	}

	static HRESULT WINAPI CreateInstance(_COM_Outptr_ CComObject<Base>** pp) noexcept;
};

template <class Base>
HRESULT WINAPI CComObject<Base>::CreateInstance(
	_COM_Outptr_ CComObject<Base>** pp) noexcept
{
	ATLASSERT(pp != NULL);
	if (pp == NULL)
		return E_POINTER;
...
public:
	...
	static _Success_(return == S_OK) HRESULT WINAPI CreateInstance(
		_COM_Outptr_ CComObjectCached<Base>** pp) noexcept;
};
...
public:
	...
	//DECLARE_GET_CONTROLLING_UNKNOWN()
	IUnknown* GetControllingUnknown() noexcept
	{
#if defined(_ATL_DEBUG_INTERFACES) && !defined(_ATL_STATIC_LIB_IMPL)
		IUnknown* p;
