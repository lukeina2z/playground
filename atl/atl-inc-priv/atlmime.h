...
#ifndef _ATL_MAX_ALLOWED_BUFFER_SIZE
#define _ATL_MAX_ALLOWED_BUFFER_SIZE 10000
#endif // _ATL_MAX_ALLOWED_BUFFER_SIZE
...
inline size_t SetRfc822Time(__out_ecount_part_z_opt(dwLen, return) LPSTR szDate, __in size_t dwLen) noexcept
{
...
inline DWORD GetContentTypeFromFileName(LPCTSTR szFileName, CSimpleString& strContentType) noexcept
{
	if (szFileName == NULL)
	{
...
public:
	...
	virtual inline BOOL GetData(CStringA& buffer, LPCSTR szBoundary, DWORD dwFlags = 0) = 0;
...
public:
	...
	inline BOOL Initialize(IMultiLanguage* pMultiLanguage = NULL) noexcept
	{
	...
	virtual inline LPCSTR GetContentType() noexcept
	{
		return "text/plain";
	}

	// Get the character set
	virtual inline LPCSTR GetCharset() noexcept
	{
		return "iso-8859-1";
	}
	...
	inline BOOL SetPriority(ATL_MIME_PRIORITY nPriority) noexcept
	{
		if (nPriority < 0)
			return FALSE;
	...
	inline ATL_MIME_PRIORITY GetPriority() noexcept
	{
		return m_nPriority;
	}

	// Set the display (friendly) name for the header
	inline BOOL SetSenderName(LPCTSTR szName, UINT uiCodePage = 0) noexcept
	{
		if (szName == NULL)
			return FALSE;
	...
	inline LPCSTR GetSenderName() noexcept
	{
		return m_strSenderName;
	}

	inline BOOL AppendUserDefinedHeader2(CStringA szHeaderName, CStringA szHeader) noexcept
	{	
		m_XHeader += szHeaderName;
		m_XHeader += ": ";
		m_XHeader = m_XHeader + szHeader;
		m_XHeader += "\r\n";
	...
	inline BOOL AppendUserDefinedHeader(LPCTSTR szHeaderName, LPCTSTR szHeader, BOOL encode = TRUE, UINT uiCodePage = 0) noexcept
	{
		...
		_ATLTRY
		{
			...
			if (!encode)
			{
				...
				m_XHeader.Append(": ", 2);
				m_XHeader = m_XHeader + szHeader;
				m_XHeader.Append((const char *)szHeader, ((CStringW)szHeader).GetLength());
				m_XHeader.Append("\r\n", 2);
			}
			else
			{
				...
				bRet = AtlMimeConvertString(m_spMultiLanguage, uiCodePage, szHeader, &szName, &nLen);
				if (bRet)
				{
					// get the charset
					...
					if (bRet)
					{
						...
						m_XHeader += CT2CA(szHeaderName);
						m_XHeader.Append(": ", 2);
						m_XHeader.Append((const char *)szHeader, ((CStringA)szHeader).GetLength());
	...
	inline BOOL AddRecipient(LPCTSTR szAddress, LPCTSTR szName = NULL, UINT uiCodePage = 0) noexcept
	{
		return AddRecipientHelper(m_strTo, szAddress, szName, uiCodePage);
	}

	// Get the recipients string ("To:" line)
	inline LPCSTR GetRecipients() noexcept
	{
		return m_strTo;
	}

	// Clear all recipients ("To:" line)
	inline BOOL ClearRecipients() noexcept
	{
		m_strTo.Empty();
		return TRUE;
	}
	...
	inline BOOL AddCc(LPCTSTR szAddress, LPCTSTR szName = NULL, UINT uiCodePage = 0) noexcept
	{
		return AddRecipientHelper(m_strCc, szAddress, szName, uiCodePage);
	}

	// Get the recipients string ("CC:" line)
	inline LPCSTR GetCc() noexcept
	{
		return m_strCc;
	}

	// Clear the recipients string ("CC:" line)
	inline BOOL ClearCc() noexcept
	{
		m_strCc.Empty();
		return TRUE;
	}
	...
	inline BOOL AddBcc(LPCTSTR szAddress) noexcept
	{
		if (szAddress == NULL)
		{
			return FALSE;
	...
	inline LPCSTR GetBcc() noexcept
	{
		return m_strBcc;
	}

	// Clear the recipients string (Bcc part)
	inline BOOL ClearBcc() noexcept
	{
		m_strBcc.Empty();
		return TRUE;
	}
	...
	inline DWORD GetRequiredRecipientsStringLength() noexcept
	{
	...
	ATL_NOINLINE BOOL GetRecipientsString(__out_ecount_part_z(*pdwLen, *pdwLen) LPSTR szRecip, __inout LPDWORD pdwLen) noexcept
	{
		if (szRecip == NULL)
		{
			return FALSE;
	...
	// Get the sender
	inline LPCSTR GetSender() noexcept
	{
		return m_strFrom;
	}

	// Set the sender
	inline BOOL SetSender(LPCTSTR szSender) noexcept
	{
		if (szSender == NULL)
			return FALSE;
	...
	// Set the subject
	inline BOOL SetSubject(LPCTSTR szSubject, UINT uiCodePage = 0) noexcept
	{
		if (szSubject == NULL)
			return FALSE;
	...
	// Get the subject
	inline LPCSTR GetSubject() noexcept
	{
		return (LPCSTR)m_strSubject;
	}

	// Dump the header to hFile
    virtual inline BOOL GetData(CStringA& buffer, LPCSTR, DWORD dwFlags = 0) noexcept
	{
		DWORD dwOffset = 0;
		int nMaxSendLen = GetRequiredBufferSize(ATLSMTP_MAX_LINE_LENGTH-4);
		
		char spSendBuffer[1000] = "";
	...
	// Dump the header to hFile
	virtual inline BOOL WriteData(HANDLE hFile, LPOVERLAPPED pOverlapped, LPCSTR /*szBoundary*/, DWORD dwFlags = 0) noexcept
	{
		if (pOverlapped == NULL)
		{
			return FALSE;
...
protected:
	...
	virtual inline BOOL MakeMimeHeader(CStringA& /*header*/, LPCSTR /*szBoundary*/) noexcept
	{
	...
	inline BOOL GetEncodedString(__in CStringA& headerString, __in LPCSTR szCharset, __out_ecount_part_z(nBufLen, dwLength) LPSTR szBuf, __in int nBufLen, __out DWORD& dwLength, __out BOOL& bEncoded) noexcept
	{
//		BOOL bEncoded = FALSE;
		bEncoded = FALSE;
	...
	inline BOOL AddRecipientHelper(CStringA& str, LPCTSTR szAddress, LPCTSTR szName = NULL, UINT uiCodePage = 0) noexcept
	{
	...
	inline BOOL FormatField(LPBYTE pbSrcData, int nSrcLen, LPBYTE pbDest, 
		DWORD* pnBufLen, DWORD dwFlags = 0) noexcept
	{
		if(pnBufLen == NULL)
			return FALSE;
	...
	inline int GetRequiredBufferSize(int nMaxLineLength) noexcept
	{
...
public:
	...
	virtual ~CMimeAttachment() noexcept
	{
	}
	...
	inline BOOL SetEncodingScheme(int nScheme) noexcept
	{
	...
	inline BOOL SetContentType(LPCTSTR szContent) noexcept
	{
		_ATLTRY
		{
...
public:
	...
	inline BOOL Initialize(LPCTSTR szFileName, LPCTSTR szDisplayName = NULL, IMultiLanguage* pMultiLanguage = NULL, UINT uiCodePage = 0) noexcept
	{
	...
	virtual inline BOOL GetData(CStringA& buffer, LPCSTR szBoundary, DWORD dwFlags = 0) noexcept
	{
		char szBuffer[_ATL_MAX_ALLOWED_BUFFER_SIZE] = "";
		...
		strncat(szBuffer, header, header.GetLength());
...
// if double buffering is defined, create two buffers
		...
		do
		{
			...
			if (bRet)
			{
				strncat(szBuffer, currBuffer, nEncodedLength);
			}
...
public:
	...
	inline BOOL Initialize(void* pData, DWORD nDataLength, BOOL bCopyData = TRUE, LPCTSTR szDisplayName = NULL, 
		IMultiLanguage* pMultiLanguage = NULL, UINT uiCodePage = 0) noexcept
	{
    ...
    virtual inline BOOL GetData(CStringA&, LPCSTR, DWORD = 0) noexcept
    {
        return FALSE;
    }
...
public:
	...
	inline BOOL Initialize(LPCTSTR szText, int nTextLen = -1, IMultiLanguage* pMultiLanguage = NULL, UINT uiCodePage = 0) noexcept
	{
		BOOL bRet = TRUE;
	...
	virtual inline BOOL GetData(CStringA& buffer, LPCSTR szBoundary, DWORD dwFlags = 0) noexcept
	{
		...
		//subtract 2 from these because it's easier for when we have
		//to break lines with a CRLF
		int nMaxLineLength = ATLSMTP_MAX_LINE_LENGTH-2; // Todo: max size
		int nMaxBufferSize = ATLSMTP_READBUFFER_SIZE-2;
...
public:
	CMimeMessage(IMultiLanguage *pMultiLanguage = NULL) noexcept
	{
		Initialize(pMultiLanguage);
		Checked::memcpy_s(m_szDisplayName, MAX_PATH+1, ATLMIME_EMAIL, sizeof(ATLMIME_EMAIL));
	}

	virtual ~CMimeMessage() noexcept
	{
		RemoveParts();
	}

	void RemoveParts() noexcept
	{
	...
	inline BOOL SetDisplayName(LPCTSTR szDisplayName) noexcept
	{
	...
	// uiCodePage - the codepage (optional)
	inline BOOL AddText(LPCTSTR szText, int nTextLen = -1, int nPos = 1, UINT uiCodePage = 0) noexcept
	{
		if (szText == NULL)
			return FALSE;
	...
	virtual inline BOOL GetData(CStringA& buffer, LPCSTR szBoundary, DWORD dwFlags = 0) noexcept
	{
		...
		CStringA tempBuffer;
		if (!CMimeHeader::GetData(tempBuffer, szBoundaryBuf, dwFlags))
			return FALSE;
		...
		while (currPos != NULL)
		{
			...
			if (!pCurrPart->GetData(bodyBuffer, szBoundaryBuf, dwFlags))
			{
				return FALSE;
			}
	...
	// Dump the data
	virtual BOOL WriteData(HANDLE hFile, LPOVERLAPPED pOverlapped, LPCSTR szBoundary=NULL, DWORD dwFlags = 0) noexcept
	{			
		if (pOverlapped == NULL)
		{
			return FALSE;
