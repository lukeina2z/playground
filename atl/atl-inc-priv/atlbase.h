...
// Private to avoid accidental use
	CComCritSecLock(_In_ const CComCritSecLock&) noexcept;
	CComCritSecLock& operator=(_In_ const CComCritSecLock&) noexcept;
};
...
template< class TLock >
_When_(this->m_bLocked != 0, _Requires_lock_held_(this->m_cs) _Releases_lock_(this->m_cs) _Post_satisfies_(this->m_bLocked == 0))
inline CComCritSecLock< TLock >::~CComCritSecLock() noexcept
{
	if( m_bLocked )
	{
		Unlock();
...
inline HRESULT CComCritSecLock< TLock >::Lock() noexcept
{
	HRESULT hr;
...
inline void CComCritSecLock< TLock >::Unlock() noexcept
{
	ATLASSUME( m_bLocked );
...
{
public:
	static ULONG WINAPI Increment(_Inout_ LPLONG p) noexcept
	{
		return ::InterlockedIncrement(p);
	}
	static ULONG WINAPI Decrement(_Inout_ LPLONG p) noexcept
	{
...
{
public:
	CAutoVectorPtr() noexcept :
		m_p( NULL )
	{
	}
	CAutoVectorPtr(_Inout_ CAutoVectorPtr< T >& p) noexcept
	{
		m_p = p.Detach();  // Transfer ownership
	}
	explicit CAutoVectorPtr(_Pre_maybenull_ T* p) noexcept :
		m_p( p )
	{
	}
	~CAutoVectorPtr() noexcept
	...
	CAutoVectorPtr< T >& operator=(_Inout_ CAutoVectorPtr< T >& p) noexcept
	{
		if(*this==p)
		{
			if(m_p == NULL)
	...
	// Allocate the vector
	bool Allocate(_In_ size_t nElements) noexcept
	{
		ATLASSUME( m_p == NULL );
	...
	void Attach(_In_opt_ T* p) noexcept
	{
		ATLASSUME( m_p == NULL );
		m_p = p;
	}
...
public:
	...
	CAutoPtr(_Inout_ CAutoPtr< TSrc >& p) noexcept
	{
		m_p = p.Detach();  // Transfer ownership
	}
	CAutoPtr(_Inout_ CAutoPtr< T >& p) noexcept
	{
		m_p = p.Detach();  // Transfer ownership
	}
	explicit CAutoPtr(_In_opt_ T* p) noexcept :
		m_p( p )
	{
	}
	~CAutoPtr() noexcept
	...
	template< typename TSrc >
	CAutoPtr< T >& operator=(_Inout_ CAutoPtr< TSrc >& p) noexcept
	{
		if(m_p==p.m_p)
		{
	...
	CAutoPtr< T >& operator=(_Inout_ CAutoPtr< T >& p) noexcept
	{
		if(*this==p)
		{
			if(this!=&p)
...
{
public:
	CAutoStackPtr() noexcept :
		m_p( NULL )
	{
	}
	template< typename TSrc >
	CAutoStackPtr(_Inout_ CAutoStackPtr< TSrc >& p) noexcept
	{
		m_p = p.Detach();  // Transfer ownership
	}
	CAutoStackPtr(_Inout_ CAutoStackPtr< T >& p) noexcept
	{
		m_p = p.Detach();  // Transfer ownership
	}
	explicit CAutoStackPtr(_In_opt_ T* p) noexcept :
		m_p( p )
	{
	}
	~CAutoStackPtr() noexcept
	...
	// Templated version to allow pBase = pDerived
	template< typename TSrc >
	CAutoStackPtr< T >& operator=(_Inout_ CAutoStackPtr< TSrc >& p) noexcept
	{
		if(m_p==p.m_p)
		{
			// This means that two CAutoPtrs of two different types had the same m_p in them
	...
	CAutoStackPtr< T >& operator=(_Inout_ CAutoStackPtr< T >& p) noexcept
	{
		if(*this==p)
		{
			if(this!=&p)
...
Dest* static_cast_auto(_In_ const CAutoPtr< Src >& pSrc) noexcept
{
...
public:
	...
	static void* Allocate(_In_ size_t nBytes) noexcept
	{
#ifdef _WIN64
		if( nBytes > INT_MAX )
		{
	...
	static void Free(_In_opt_ void* p) noexcept
	{
		::CoTaskMemFree(p);
	}
};
...
{
public:
	CComHeapPtr() noexcept
	{
	}

	explicit CComHeapPtr(_In_ T* pData) noexcept :
...
public:
	CHandle() noexcept;
	CHandle(_Inout_ CHandle& h) noexcept;
	explicit CHandle(_In_ HANDLE h) noexcept;
	~CHandle() noexcept;

	CHandle& operator=(_Inout_ CHandle& h) noexcept;
...
inline CHandle::CHandle() noexcept :
	m_h( NULL )
{
}

inline CHandle::CHandle(_Inout_ CHandle& h) noexcept :
	m_h( NULL )
{
	Attach( h.Detach() );
}

inline CHandle::CHandle(_In_ HANDLE h) noexcept :
	m_h( h )
{
}
...
inline CHandle& CHandle::operator=(_Inout_ CHandle& h) noexcept
{
	if( this != &h )
	{
		if( m_h != NULL )
...
inline CHandle::operator HANDLE() const noexcept
{
	return( m_h );
}

inline void CHandle::Attach(_In_ HANDLE h) noexcept
{
	ATLASSUME( m_h == NULL );
	m_h = h;  // Take ownership
}

inline HANDLE CHandle::Detach() noexcept
{
	HANDLE h;
...
inline void CHandle::Close() noexcept
{
	if( m_h != NULL )
	{
		::CloseHandle( m_h );
...
// Private to avoid accidental use
	CCritSecLock(_In_ const CCritSecLock&) noexcept;
	CCritSecLock& operator=(_In_ const CCritSecLock&) noexcept;
};
...
public:
	...
	CRegKey(_In_opt_ CAtlTransactionManager* pTM = NULL) noexcept;
	CRegKey(_Inout_ CRegKey& key) noexcept;
	explicit CRegKey(_In_ HKEY hKey) noexcept;
	~CRegKey() noexcept;

	CRegKey& operator=(_Inout_ CRegKey& key) noexcept;
...
public:
	...
	LSTATUS SetGUIDValue(
		_In_opt_z_ LPCTSTR pszValueName,
		_In_ REFGUID guidValue) noexcept;
	...
	LSTATUS SetMultiStringValue(
		_In_opt_z_ LPCTSTR pszValueName,
		_In_z_ LPCTSTR pszValue) noexcept;
	...
	LSTATUS SetKeyValue(
		...
		_In_opt_z_ LPCTSTR lpszValueName = NULL) noexcept;
	...
	LSTATUS EnumKey(
		...
		_Out_opt_ FILETIME* pftLastWriteTime = NULL) noexcept;
	...
	LSTATUS RecurseDeleteKey(_In_z_ LPCTSTR lpszKey) noexcept;
...
{
public:
	CAtlDebugInterfacesModule() noexcept :
		m_nIndexQI( 0 ),
		m_nIndexBreakAt( 0 )
	{
		if (FAILED(m_cs.Init()))
	...
	HRESULT AddNonAddRefThunk(
		_In_ IUnknown* p,
		_In_z_ LPCTSTR lpsz,
		_Outptr_ IUnknown** ppThunkRet) noexcept
	{
		if (ppThunkRet == NULL)
			return E_POINTER;
		*ppThunkRet = NULL;
	...
	void DeleteNonAddRefThunk(_In_ IUnknown* pUnk) noexcept
	{
		CComCritSecLock<CComCriticalSection> lock(m_cs, false);
		HRESULT hr = lock.Lock();
		if (FAILED(hr))
	...
	void DeleteThunk(_In_ _QIThunk* p) noexcept
	{
		CComCritSecLock<CComCriticalSection> lock(m_cs, false);
		HRESULT hr = lock.Lock();
		if (FAILED(hr))
...
public :
	...
	HRESULT AddTermFunc(
		_In_ _ATL_TERMFUNC* pFunc,
		_In_ DWORD_PTR dw) noexcept
	{
		return AtlModuleAddTermFunc(this, pFunc, dw);
	}

	virtual HRESULT GetGITPtr(_Outptr_ IGlobalInterfaceTable** ppGIT) noexcept
	{
...
#ifdef _ATL_USE_WINAPI_FAMILY_DESKTOP_APP
	virtual HRESULT AddCommonRGSReplacements(_Inout_ IRegistrarBase* /*pRegistrar*/) noexcept = 0;
	...
	HRESULT WINAPI UpdateRegistryFromResource(
		...
		_In_ BOOL bRegister,
		_In_opt_ struct _ATL_REGMAP_ENTRY* pMapEntries = NULL) noexcept;
	HRESULT WINAPI UpdateRegistryFromResource(
		_In_ UINT nResID,
		_In_ BOOL bRegister,
		_In_opt_ struct _ATL_REGMAP_ENTRY* pMapEntries = NULL) noexcept;
...
#define DECLARE_LIBID(libid) \
	static void InitLibId() noexcept \
	{ \
		ATL::CAtlModule::m_libid = libid; \
	}

#define DECLARE_REGISTRY_APPID_RESOURCEID(resid, appid) \
	static LPCOLESTR GetAppId() noexcept \
	{ \
		return OLESTR(appid); \
	} \
	static const TCHAR* GetAppIdT() noexcept \
	{ \
		return _T(appid); \
	} \
	static HRESULT WINAPI UpdateRegistryAppId(_In_ BOOL bRegister) noexcept \
	{ \
...
inline HRESULT AtlGetGITPtr(_Outptr_ IGlobalInterfaceTable** ppGIT) noexcept
{
	if (ppGIT == NULL)
		return E_POINTER;
...
public :
	...
	HRESULT RegisterServer(
		_In_ BOOL bRegTypeLib = FALSE,
		_In_opt_ const CLSID* pCLSID = NULL) noexcept
	{
		(pCLSID);
		(bRegTypeLib);
	...
	HRESULT RegisterAppId() noexcept
	{
		return T::UpdateRegistryAppId(TRUE);
	}

	HRESULT UnregisterAppId() noexcept
	{
		return T::UpdateRegistryAppId(FALSE);
	}

	virtual HRESULT AddCommonRGSReplacements(_Inout_ IRegistrarBase* pRegistrar) noexcept
	{
...
#pragma warning(pop)  // disable 4483
	...
	BOOL WINAPI DllMain(
		_In_ DWORD dwReason,
		_In_opt_ LPVOID lpReserved) noexcept;
	...
	HRESULT DllCanUnloadNow() noexcept
	{
	...
	HRESULT DllRegisterServer(
		_In_ BOOL bRegTypeLib = TRUE) noexcept
	{
	...
	HRESULT DllUnregisterServer(
		_In_ BOOL bUnRegTypeLib = TRUE) noexcept
	{
		LCID lcid = GetThreadLocale();
		SetThreadLocale(LOCALE_SYSTEM_DEFAULT);
		T* pT = static_cast<T*>(this);
...
public :
	...
	static HRESULT InitializeCom() noexcept
	{
	...
	static void UninitializeCom() noexcept
	{
		CoUninitialize();
	}
	...
	static DWORD WINAPI MonitorProc(_In_ void* pv) noexcept
	{
	...
	bool ParseCommandLine(
		...
		_Out_ HRESULT* pnRetCode) noexcept
	{
		*pnRetCode = S_OK;
	...
	HRESULT Run(_In_ int nShowCmd = SW_HIDE) noexcept
	{
		HRESULT hr = S_OK;
	...
	HRESULT RevokeClassObjects() noexcept
	{
...
public :
	...
	inline HRESULT RegisterAppId(_In_ bool bService = false) noexcept
	{
		if (!Uninstall())
			return E_FAIL;
	...
	void ServiceMain(
		_In_ DWORD dwArgc,
		_In_reads_(dwArgc) _Deref_pre_z_ LPTSTR* lpszArgv) noexcept
	{
		lpszArgv;
		dwArgc;
	...
	void OnContinue() noexcept
	{
	}

	void OnInterrogate() noexcept
	{
	}
	...
	void OnUnknownRequest(_In_ DWORD /*dwOpcode*/) noexcept
	{
		LogEvent(_T("Bad service request"));
	}

	void Handler(_In_ DWORD dwOpcode) noexcept
	{
		T* pT = static_cast<T*>(this);
...
#endif
#if 0
                sergeid, 26/Feb/03. We can not have a pop up box in case
                there are problems with  installation / uninstallation of service
                as this will kill automatic deployment process. We will log an event
	...
	void LogEventEx(
		...
		_In_ WORD type = EVENTLOG_INFORMATION_TYPE) noexcept
	{
...
#pragma warning(disable : 4793)
	virtual void __cdecl LogEvent(
		_In_z_ _Printf_format_string_ LPCTSTR pszFormat, ...) noexcept
	{
...
#pragma warning(pop)  // disable 4793
	...
	void SetServiceStatus(_In_ DWORD dwState) noexcept
	{
...
protected:
	...
	static void WINAPI _Handler(_In_ DWORD dwOpcode) noexcept
	{
...
#pragma warning(pop)  // disable 4483
	...
	HINSTANCE& get_m_hInst() const noexcept
	{
	...
	HINSTANCE& get_m_hInstResource() const noexcept
	{
		return _AtlBaseModule.m_hInstResource;
	}
	void put_m_hInstResource(_In_ HINSTANCE h) noexcept
	{
		_AtlBaseModule.SetResourceInstance(h);
	}
	HINSTANCE SetResourceInstance(_In_ HINSTANCE h) noexcept
	{
		return _AtlBaseModule.SetResourceInstance(h);
	}

	HINSTANCE GetModuleInstance() noexcept
	{
		return _AtlBaseModule.m_hInst;
	}
	HINSTANCE GetResourceInstance() noexcept
	{
		return _AtlBaseModule.m_hInstResource;
	}

	__declspec(property(get = get_m_hInstTypeLib, put = put_m_hInstTypeLib)) HINSTANCE m_hInstTypeLib;
	HINSTANCE& get_m_hInstTypeLib() const noexcept
	{
		return _AtlComModule.m_hInstTypeLib;
	}
	void put_m_hInstTypeLib(_In_ HINSTANCE h) noexcept
	{
		_AtlComModule.m_hInstTypeLib = h;
	}

	HINSTANCE GetTypeLibInstance() const noexcept
	{
		return _AtlComModule.m_hInstTypeLib;
	}
	...
	CRITICAL_SECTION& get_m_csWindowCreate() noexcept;

		__declspec(property(get  = get_m_pCreateWndList, put = put_m_pCreateWndList)) _AtlCreateWndData* m_pCreateWndList;
	_AtlCreateWndData*& get_m_pCreateWndList() noexcept;
	void put_m_pCreateWndList(_In_ _AtlCreateWndData* p) noexcept;
#endif // _ATL_NO_WIN_SUPPORT

	__declspec(property(get  = get_m_csObjMap)) CRITICAL_SECTION m_csObjMap;
	CRITICAL_SECTION& get_m_csObjMap() noexcept;
	...
	UINT& get_m_nIndexQI() noexcept;
	void put_m_nIndexQI(_In_ UINT nIndex) noexcept;

	__declspec(property(get  = get_m_nIndexBreakAt, put = put_m_nIndexBreakAt)) UINT m_nIndexBreakAt;
	UINT& get_m_nIndexBreakAt() noexcept;
	void put_m_nIndexBreakAt(_In_ UINT nIndex) noexcept;

	__declspec(property(get  = get_m_paThunks)) CSimpleArray<_QIThunk*>* m_paThunks;
	CSimpleArray<_QIThunk*>* get_m_paThunks() noexcept;
	HRESULT AddThunk(
		_Inout_ _Deref_pre_valid_ _Deref_post_valid_ IUnknown** pp,
		_In_z_ LPCTSTR lpsz,
		_In_ REFIID iid) noexcept;
	...
	void DeleteNonAddRefThunk(_In_ IUnknown* pUnk) noexcept;
	void DeleteThunk(_In_ _QIThunk* p) noexcept;
	bool DumpLeakedThunks() noexcept;
#endif // _ATL_DEBUG_INTERFACES

	HRESULT Init(
		_In_ _ATL_OBJMAP_ENTRY* p,
		_In_ HINSTANCE h,
		_In_opt_ const GUID* plibid = NULL) noexcept;
...
#ifdef _ATL_USE_WINAPI_FAMILY_DESKTOP_APP
	// Register/Revoke All Class Factories with the OS (EXE only)
	HRESULT RegisterClassObjects(_In_ DWORD dwClsContext, _In_ DWORD dwFlags) noexcept;
	HRESULT RevokeClassObjects() noexcept;

	// Registry support (helpers)
	HRESULT RegisterTypeLib() noexcept;
	HRESULT RegisterTypeLib(_In_z_ LPCTSTR lpszIndex) noexcept;
	HRESULT UnRegisterTypeLib() noexcept;
	HRESULT UnRegisterTypeLib(_In_z_ LPCTSTR lpszIndex) noexcept;
	...
	HRESULT UnregisterServer(
		_In_ BOOL bUnRegTypeLib,
		_In_opt_ const CLSID* pCLSID = NULL) noexcept;
	HRESULT UnregisterServer(_In_opt_ const CLSID* pCLSID = NULL) noexcept;
	...
	HRESULT DllRegisterServer(_In_ BOOL bRegTypeLib = TRUE)  noexcept
	{
		// registers object, typelib and all interfaces in typelib
		return RegisterServer(bRegTypeLib);
	}

	HRESULT DllUnregisterServer(_In_ BOOL bUnRegTypeLib = TRUE)  noexcept
	{
	...
	HRESULT CreateInstance(
		...
		_In_ REFIID /*riid*/,
		_COM_Outptr_ void** /*ppvObj*/) noexcept
	{
		ATLASSERT(0);
...
public:
	static HANDLE CreateThread(
		...
		_Out_opt_ DWORD *pdwThreadId) noexcept
	{
...
public:
	...
	explicit CComGITPtr(_In_ DWORD dwCookie) noexcept
	{
	...
	CComGITPtr(_Inout_ CComGITPtr&& rv) noexcept
	{
		m_dwCookie = rv.m_dwCookie;
		rv.m_dwCookie = 0;
	}

	CComGITPtr& operator=(_Inout_ CComGITPtr&& rv) noexcept
	{
		if (this != &rv)
		{
	...
	HRESULT Attach(_In_ T* p) noexcept
	{
		if (p)
		{
	...
	HRESULT Attach(_In_ DWORD dwCookie) noexcept
	{
	...
	HRESULT CopyTo(_Outptr_ T** pp) const noexcept
	{
		CComPtr<IGlobalInterfaceTable> spGIT;
		HRESULT hr = E_FAIL;
		hr = AtlGetGITPtr(&spGIT);
...
inline CRegKey::CRegKey(_In_opt_ CAtlTransactionManager* pTM) noexcept :
	m_hKey( NULL ), m_samWOW64(0), m_pTM(pTM)
{
}

inline CRegKey::CRegKey(_Inout_ CRegKey& key) noexcept :
	m_hKey( NULL )
{
...
inline CRegKey::CRegKey(_In_ HKEY hKey) noexcept :
	m_hKey(hKey), m_samWOW64(0), m_pTM(NULL)
{
}

inline CRegKey::~CRegKey() noexcept
{Close();}

inline CRegKey& CRegKey::operator=(_Inout_ CRegKey& key) noexcept
{
...
inline CRegKey::operator HKEY() const noexcept
{
	return m_hKey;
}

inline HKEY CRegKey::Detach() noexcept
{
	HKEY hKey = m_hKey;
	m_hKey = NULL;
	m_samWOW64 = 0;
...
inline void CRegKey::Attach(_In_ HKEY hKey) noexcept
{
...
inline LSTATUS CRegKey::DeleteSubKey(_In_z_ LPCTSTR lpszSubKey) noexcept
{
	ATLASSUME(m_hKey != NULL);
...
inline LSTATUS CRegKey::DeleteValue(_In_z_ LPCTSTR lpszValue) noexcept
{
	ATLASSUME(m_hKey != NULL);
	return RegDeleteValue(m_hKey, (LPTSTR)lpszValue);
}

inline LSTATUS CRegKey::Close() noexcept
{
...
inline LSTATUS CRegKey::Flush() noexcept
{
	ATLASSUME(m_hKey != NULL);
...
inline LSTATUS CRegKey::SetKeyValue(
	_In_z_ LPCTSTR lpszKeyName,
	_In_opt_z_ LPCTSTR lpszValue,
	_In_opt_z_ LPCTSTR lpszValueName) noexcept
{
	ATLASSERT(lpszValue != NULL);
	CRegKey key;
...
inline LSTATUS CRegKey::RecurseDeleteKey(_In_z_ LPCTSTR lpszKey) noexcept
{
	CRegKey key;
...
inline void _QIThunk::Dump() noexcept
{
	TCHAR buf[512+1];
	if (m_dwRef != 0)
	{
...
static inline UINT WINAPI AtlGetDirLen(_In_z_ LPCOLESTR lpszPathName) noexcept
{
...
inline CRITICAL_SECTION& CComModule::get_m_csWindowCreate() noexcept
{
	return _AtlWinModule.m_csWindowCreate.m_sec;
}

inline _AtlCreateWndData*& CComModule::get_m_pCreateWndList()  noexcept
{
	return _AtlWinModule.m_pCreateWndList;
}
inline void CComModule::put_m_pCreateWndList(_In_ _AtlCreateWndData* p) noexcept
{
...
inline CRITICAL_SECTION& CComModule::get_m_csObjMap() noexcept
{
	return _AtlComModule.m_csObjMap.m_sec;
}

inline CRITICAL_SECTION& CComModule::get_m_csStaticDataInit() noexcept
{
	return m_csStaticDataInitAndTypeInfo.m_sec;
}

#ifdef _ATL_DEBUG_INTERFACES
inline UINT& CComModule::get_m_nIndexQI() noexcept
{
	return _AtlDebugInterfacesModule.m_nIndexQI;
}
inline void CComModule::put_m_nIndexQI(_In_ UINT nIndex) noexcept
{
	_AtlDebugInterfacesModule.m_nIndexQI = nIndex;
}
inline UINT& CComModule::get_m_nIndexBreakAt() noexcept
{
	return _AtlDebugInterfacesModule.m_nIndexBreakAt;
}
inline void CComModule::put_m_nIndexBreakAt(_In_ UINT nIndex) noexcept
{
	_AtlDebugInterfacesModule.m_nIndexBreakAt = nIndex;
}
inline CSimpleArray<_QIThunk*>* CComModule::get_m_paThunks() noexcept
{
...
inline void CComModule::DeleteNonAddRefThunk(_In_ IUnknown* pUnk) noexcept
{
	_AtlDebugInterfacesModule.DeleteNonAddRefThunk(pUnk);
}

inline void CComModule::DeleteThunk(_In_ _QIThunk* p) noexcept
{
	_AtlDebugInterfacesModule.DeleteThunk(p);
}

inline bool CComModule::DumpLeakedThunks() noexcept
{
...
inline HRESULT CComModule::Init(
	_In_ _ATL_OBJMAP_ENTRY* p,
	_In_ HINSTANCE /*h*/,
	_In_opt_ const GUID* plibid) noexcept
{
	if (plibid != NULL)
		m_libid = *plibid;
...
inline void CComModule::Term() noexcept
{
	_ATL_OBJMAP_ENTRY* pEntry;
	if (m_pObjMap != NULL)
	{
...
inline HRESULT CComModule::RevokeClassObjects() noexcept
{
	HRESULT hr = S_OK;
	_ATL_OBJMAP_ENTRY* pEntry;
	if (m_pObjMap != NULL)
...
// Registry support (helpers)
inline HRESULT CComModule::RegisterTypeLib() noexcept
{
	return _AtlComModule.RegisterTypeLib();
}
inline HRESULT CComModule::RegisterTypeLib(_In_z_ LPCTSTR lpszIndex) noexcept
{
	return _AtlComModule.RegisterTypeLib(lpszIndex);
}
inline HRESULT CComModule::UnRegisterTypeLib() noexcept
{
	return _AtlComModule.UnRegisterTypeLib();
}
inline HRESULT CComModule::UnRegisterTypeLib(_In_z_ LPCTSTR lpszIndex) noexcept
{
...
inline HRESULT CComModule::UnregisterServer(_In_opt_ const CLSID* pCLSID /*= NULL*/) noexcept
{
