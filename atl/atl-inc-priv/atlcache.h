...
public:
	...
	virtual void OnDestroyEntry(const typename baseClass::NodeType * pEntry)
	{
		ATLASSERT(pEntry);
		if (!pEntry)
			return;
...
public:
	...
	HINSTANCE Load(LPCSTR szDllName, void *pPeerInfo)
	{
		...
		//  before loading the lib revert to self
		HANDLE  hToken = NULL;
...
//IStencilCache methods
	...
	virtual void OnDestroyEntry(const typename cacheBase::NodeType * pEntry)
	{
		ATLASSERT(pEntry);
		if (!pEntry)
			return;
