...
public:
	...
	void Free(_Inout_ CStringData* pData) noexcept;
	...
	CStringData* GetNilString() noexcept;
	IAtlStringMgr* Clone() noexcept;
};
...
private:
	CStaticString(_In_ const CStaticString& str) noexcept;
	CStaticString& operator=(_In_ const CStaticString& str) noexcept;
};
...
public:
		...
		_In_ int nLength)
	{
        ...
        //
        // MSA 'FIX': it is unfortunate, but MSA code uses CStringA to carry
        // binary data, this check below breaks that. so until MSA makes 
        // a comprehensive change to move to a better object, we'll remove
        // the below check.
        // 
...
private:
	CStrBufT(_In_ const CStrBufT&) noexcept;
	CStrBufT& operator=(_In_ const CStrBufT& ) noexcept;
};
