...
{
public:
	CAToREChar(const char* psz) noexcept
	: m_psz(psz)
	{
	}
	operator const RECHARTYPE*() const noexcept { return m_psz; }
	const char* m_psz;
};
...
public:
	...
	operator const wchar_t*() const noexcept { return (wchar_t*)m_a2w; }
...
public:
	...
	static size_t GetBitFieldForRangeArrayIndex(const RECHARTYPE *sz) noexcept
	{
	...
	static RECHARTYPE *Next(const RECHARTYPE *sz) noexcept
	{
		return (RECHARTYPE *) (sz+1);
	}

	static int Strncmp(const RECHARTYPE *szLeft, const RECHARTYPE *szRight, size_t nCount) noexcept
	{
		return strncmp(szLeft, szRight, nCount);
	}

	static int Strnicmp(const RECHARTYPE *szLeft, const RECHARTYPE *szRight, size_t nCount) noexcept
	{
		return _strnicmp(szLeft, szRight, nCount);
	}

	_ATL_INSECURE_DEPRECATE("CAtlRECharTraitsA::Strlwr must be passed a buffer size.")
	static RECHARTYPE *Strlwr(RECHARTYPE *sz) noexcept
	{
		#pragma warning (push)
	...
	static RECHARTYPE *Strlwr(RECHARTYPE *sz, int nSize) noexcept
	{
		Checked::strlwr_s(sz, nSize);
		return sz;
	}

	static long Strtol(const RECHARTYPE *sz, RECHARTYPE **szEnd, int nBase) noexcept
	{
		return strtol(sz, szEnd, nBase);
	}

	static int Isdigit(RECHARTYPE ch) noexcept
	{
	...
	static BOOL UseBitFieldForRange() noexcept
	{
		return TRUE;
	}

	static int ByteLen(const RECHARTYPE *sz) noexcept
	{
...
protected:
	BOOL Initialize(UINT uRequiredMem, UINT uNumGroups) noexcept
	{
		m_nTos = 0;
...
protected:
	...
	REParseError GetLastParseError() noexcept
	{
		return m_LastError;
	}

	void SetLastParseError(REParseError Error) noexcept
	{
		m_LastError = Error;
	}
	// CAtlRegExp::Reset
	...
	inline int InstructionsPerRangeBitField() noexcept
	{
	...
	public:
		...
		CParseState(CAtlRegExp *pRegExp) noexcept
		{
	...
	BOOL PeekToken(const RECHAR **ppszRE, int ch) noexcept
	{
		if (**ppszRE != ch)
			return FALSE;
		return TRUE;
	...
	BOOL MatchToken(const RECHAR **ppszRE, int ch) noexcept
	{
	...
	INSTRUCTION &GetInstruction(size_t nIndex) noexcept
	{
	...
	RECHAR GetEscapedChar(RECHAR ch) noexcept
	{
		if (ch == 't')
			return '\t';
		return ch;
	...
	int ParseCharItem(const RECHAR **ppszRE, RECHAR *pchStartChar, RECHAR *pchEndChar) noexcept
	{
		if (**ppszRE == '\\')
		{
			*ppszRE = CharTraits::Next(*ppszRE);
