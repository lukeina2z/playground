...
Prerequisites
    ...
    Set up C# version 7.3 and .Net version 4.72 on your machine. For instructions, consult a .NET Framework setup tutorial, like this or for .Net Core, this.
    Set up NuGet (if not already installed with your environment), which is the most popular C# dependency management tool.

Instructions

    Download the app as a ZIP file.
    Extract the app source code. sample project data: The configuration parameters defined in the kusto_sample_config.json file are preconfigured with the appropriate values for your cluster. Verify that these are correct.
    Open a command line window and navigate to the folder where you extracted the app.
    Either use an IDE of choice to build and run the project, or do the following using the command line window:
        For .net Core:
            Run dotnet build QuickStart -o OUTPUT_PATH to build the project and its dependencies into a set of binaries, including an executable that can be used to run the application. For more information, click here
        For .net Framework:
            Locate your MSBuild app and Run MSBuild build QuickStart.csproj /p:OutputPath=OUTPUT_PATH to build the project and its dependencies into a set of binaries, including an executable that can be used to run the application. For more information, click here
        Make sure the build succeeded without any errors.
        Go to your selected output path, and run the exe file from the command line window with QuickStart.exe or just double click it from the file explorer.
...
Query will only be possible after the ingestion process 
```
