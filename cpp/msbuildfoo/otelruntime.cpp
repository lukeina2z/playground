...
namespace MsaLab
{
  ...
  namespace trace_exporter = opentelemetry::exporter::trace;
    ...
    OTelRuntime::OTelRuntime(const std::string& serviceName, bool useGenevaExporter)
      : m_serviceName(serviceName), m_useGenevaExporter(useGenevaExporter)
    {
      Initialize();
    }

    OTelRuntime::~OTelRuntime()
    {
      Shutdown();
    }

    opentelemetry::nostd::shared_ptr<opentelemetry::trace::Tracer> OTelRuntime::GetTracer(const std::string& tracerName)
    {
      auto provider = opentelemetry::trace::Provider::GetTracerProvider();
      return provider->GetTracer(tracerName);
    }

    void OTelRuntime::Initialize()
    {
    ...
    opentelemetry::nostd::shared_ptr<opentelemetry::trace::TracerProvider> OTelRuntime::CreateOtlpTraceProvider(
    ...
    opentelemetry::nostd::shared_ptr<opentelemetry::trace::TracerProvider> OTelRuntime::CreateGenevaTraceProvider(const std::string& serviceName)
    {
